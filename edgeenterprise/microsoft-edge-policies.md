...
이 정책을 사용 설정하면 인트라넷 영역 HTTPS 웹사이트의 인트라넷 영역 파일에 대한 파일 URL 링크가 해당 파일 또는 디렉터리에 대한 Windows 파일 탐색기를 엽니다.
...
```
  ...
  이 설정을 사용하면 엔터프라이즈 모드 사이트 목록에 대한 사용자 지정 새로 고침 간격을 지정할 수 있습니다. 새로 고침 간격은 분 단위로 지정됩니다.
