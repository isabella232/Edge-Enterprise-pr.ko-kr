...
|[DefaultWebHidGuardSetting](#defaultwebhidguardsetting)|WebHID API의 사용 제어|
...
|[JavaScriptJitBlockedForSites](#javascriptjitblockedforsites)|JavaScript가 이러한 사이트에서 JIT를 사용하지 못하도록 차단|
...
|[WebHidAskForUrls](#webhidaskforurls)|이러한 사이트에서 WebHID API 허용|
|[WebHidBlockedForUrls](#webhidblockedforurls)|이러한 사이트에서 WebHID API 차단|
...
|[ControlDefaultStateOfAllowExtensionFromOtherStoresSettingEnabled](#controldefaultstateofallowextensionfromotherstoressettingenabled)|다른 스토어에서 확장 허용 설정의 기본 상태 구성|
...
|정책 이름|캡션|
|-|-|
|[EdgeDefaultProfileEnabled](#edgedefaultprofileenabled)|기본 프로필 설정 사용|
...
|[SignInCtaOnNtpEnabled](#signinctaonntpenabled)|로그인 클릭 투 액션 대화 상자 사용|
...
|[KioskSwipeGesturesEnabled](#kioskswipegesturesenabled)|Microsoft Edge 키오스크 모드에서 살짝 밀기 제스처 사용|
...
|[PasswordManagerBlocklist](#passwordmanagerblocklist)|암호 관리자 UI(저장 및 채우기)가 비활성화될 도메인 목록을 구성합니다.|
...
|[AccessibilityImageLabelsEnabled](#accessibilityimagelabelsenabled)|화면 읽기 프로그램 사용자가 Microsoft에서 이미지 설명을 가져올 수 있도록 허용|
...
|[AdsTransparencyEnabled](#adstransparencyenabled)|광고 투명도 기능을 사용할 수 있는지 구성|
...
|[AllowGamesMenu](#allowgamesmenu)|사용자가 게임 메뉴에 액세스할 수 있도록 허용|
...
|[AllowSyncXHRInPageDismissal](#allowsyncxhrinpagedismissal)|페이지를 해제하는 동안 페이지에서 동기 XHR 요청 전송 허용(사용하지 않음)|
...
|[AppCacheForceEnabled](#appcacheforceenabled)|AppCache 기능이 기본적으로 해제되어 있더라도 다시 사용할 수 있도록 허용(사용하지 않음)|
...
|[AutofillCreditCardEnabled](#autofillcreditcardenabled)|결제 방법에 자동 채우기 사용|
...
|[BackgroundTemplateListUpdatesEnabled](#backgroundtemplatelistupdatesenabled)|서식 파일을 사용하는 컬렉션 및 기타 기능에 대해 사용 가능한 서식 파일 목록의 백그라운드 업데이트 사용(사용되지 않음)|
...
|[ConfigureKeyboardShortcuts](#configurekeyboardshortcuts)|바로 가기 키를 사용하지 않도록 설정할 명령 목록 구성|
...
|[CrossOriginWebAssemblyModuleSharingEnabled](#crossoriginwebassemblymodulesharingenabled)|WebAssembly 모듈을 원본 간으로 보낼 수 있는지 여부를 지정(사용되지 않음)|
...
|[DoNotSilentlyBlockProtocolsFromOrigins](#donotsilentlyblockprotocolsfromorigins)|플러드 방지 보호로 자동 차단할 수 없는 프로토콜 목록 정의|
|[DownloadDirectory](#downloaddirectory)|다운로드 디렉터리 설정|
|[DownloadRestrictions](#downloadrestrictions)|다운로드 제한 허용|
|[EdgeAssetDeliveryServiceEnabled](#edgeassetdeliveryserviceenabled)|Asset Delivery Service에서 자산을 다운로드하는 기능 허용|
...
|[ForceCertificatePromptsOnMultipleMatches](#forcecertificatepromptsonmultiplematches)|"AutoSelectCertificateForUrls"를 사용하여 구성한 사이트에 대해 일치하는 인증서가 여러 개인 경우 Microsoft Edge에서 자동으로 인증서를 선택할 것인지 여부 지정(사용되지 않음)|
...
|[ForceMajorVersionToMinorPositionInUserAgent](#forcemajorversiontominorpositioninuseragent)|주 버전 99에서 사용자 에이전트 문자열 고정 사용 또는 사용 안 함|
...
|[HideRestoreDialogEnabled](#hiderestoredialogenabled)|브라우저 작동 중단 후 복원 페이지 대화 상자 숨기기|
|[HubsSidebarEnabled](#hubssidebarenabled)|허브 사이드바 표시|
...
|[InternetExplorerIntegrationCloudNeutralSitesReporting](#internetexplorerintegrationcloudneutralsitesreporting)|잠재적으로 잘못 구성된 중립 사이트 URL을 M365 관리 센터 사이트 목록 앱에 보고 구성|
|[InternetExplorerIntegrationCloudSiteList](#internetexplorerintegrationcloudsitelist)|엔터프라이즈 모드 클라우드 사이트 목록 구성|
|[InternetExplorerIntegrationCloudUserSitesReporting](#internetexplorerintegrationcloudusersitesreporting)|M365 관리 센터 사이트 목록 앱에 대한 IE 모드 사용자 목록 항목 보고 구성|
...
|[InternetExplorerModeEnableSavePageAs](#internetexplorermodeenablesavepageas)|Internet Explorer 모드로 페이지 저장 허용|
...
|[MicrosoftOfficeMenuEnabled](#microsoftofficemenuenabled)|사용자가 Microsoft Office 메뉴에 액세스할 수 있도록 허용|
...
|[PromptOnMultipleMatchingCertificates](#promptonmultiplematchingcertificates)|여러 인증서가 일치하는 경우 인증서를 선택하라는 메시지를 표시합니다.|
...
|[RelatedMatchesCloudServiceEnabled](#relatedmatchescloudserviceenabled)|페이지에서 찾기에서 관련 일치 구성|
...
|[SiteSafetyServicesEnabled](#sitesafetyservicesenabled)|사용자가 Family Safety를 구성할 수 있도록 허용|
...
|[UserAgentReduction](#useragentreduction)|사용자 에이전트 감소 사용 또는 사용 안 함|
...
|[WebSQLInThirdPartyContextEnabled](#websqlinthirdpartycontextenabled)|타사 컨텍스트에서 WebSQL을 다시 사용하도록 설정(사용되지 않음)|
...
|[WinHttpProxyResolverEnabled](#winhttpproxyresolverenabled)|Windows 프록시 해결 프로그램 사용|
...
```
  ...
  #### <a name="control-use-of-javascript-jit"></a>JavaScript JIT의 사용 제어
...
```
  ...
  #### <a name="control-use-of-the-webhid-api"></a>WebHID API의 사용 제어
...
해당 정책은 [WebHidAskForUrls](#webhidaskforurls) 및 [WebHidBlockedForUrls](#webhidblockedforurls) 정책을 사용하여 특정 URL 패턴에 대해 재정의될 수 있습니다.
...
```
  ...
  #### <a name="allow-intranet-zone-file-url-links-from-microsoft-edge-to-open-in-windows-file-explorer"></a>Microsoft Edge의 인트라넷 영역 파일 URL 링크가 Windows 파일 탐색기에서 열리도록 허용합니다.
...
해당 정책을 구성하지 않는 경우 [DefaultJavaScriptSetting](#defaultjavascriptsetting)이 모든 사이트에 적용됩니다(설정된 경우). 그렇지 않은 경우 사용자의 개인 설정이 적용됩니다.
...
```
  ...
  #### <a name="allow-javascript-to-use-jit-on-these-sites"></a>JavaScript가 이러한 사이트에서 JIT를 사용하도록 허용
...
유효한 사이트 URL 패턴에 관한 자세한 내용은 [https://go.microsoft.com/fwlink/?linkid=2095322](https://go.microsoft.com/fwlink/?linkid=2095322)을(를) 참조하세요. 별표*는 해당 정책에 허용되는 값이 아닙니다.
...
이 정책은 최상위 원본 URL에만 기반하지 않고 프레임 단위로 적용됩니다. contoso.com이 JavaScriptJitAllowedForSites 정책에 나열되어 있지만 contoso.com이 fabrikam.com을 포함하는 프레임을 로드하는 경우 contoso.com은 JavaScript JIT를 사용하도록 설정하지만 fabrikam.com은 [DefaultJavaScriptJitSetting](#defaultjavascriptjitsetting)(설정된 경우)의 정책을 사용하거나 기본적으로 JavaScript JIT를 사용하도록 설정됩니다.

사이트에 대해 이 정책을 구성하지 않으면 [DefaultJavaScriptJitSetting](#defaultjavascriptjitsetting)의 정책이 사이트에 적용되고(설정된 경우) 그렇지 않으면 사이트에 대해 Javascript JIT가 사용하도록 설정됩니다.
...
```
  ...
  #### <a name="block-javascript-from-using-jit-on-these-sites"></a>JavaScript가 이러한 사이트에서 JIT를 사용하지 못하도록 차단
...
이 정책은 최상위 원본 URL에만 기반하지 않고 프레임 단위로 적용됩니다. contoso.com이 JavaScriptJitBlockedForSites 정책에 나열되어 있지만 contoso.com이 fabrikam.com을 포함하는 프레임을 로드하는 경우 contoso.com은 JavaScript JIT를 사용하지 않도록 설정하지만 fabrikam.com은 [DefaultJavaScriptJitSetting](#defaultjavascriptjitsetting)(설정된 경우)의 정책을 사용하거나 기본적으로 JavaScript JIT를 사용하도록 설정됩니다.
...
```
  ...
  #### <a name="enable-default-legacy-samesite-cookie-behavior-setting-obsolete"></a>기본 레거시 SameSite 쿠키 동작 설정 사용(사용되지 않음)
...
레거시 쿠키 동작이 여전히 필요한 경우 [LegacySameSiteCookieBehaviorEnabledForDomainList](#legacysamesitecookiebehaviorenabledfordomainlist)를 사용하여 도메인 기준에 따라 동작을 구성합니다.
...
```
  ...
  #### <a name="choose-whether-users-can-receive-customized-background-images-and-text-suggestions-notifications-and-tips-for-microsoft-services"></a>사용자가 Microsoft 서비스에 대한 사용자 지정 배경 이미지와 텍스트, 제안, 알림 및 팁을 받을 수 있는지 여부 선택
...
```
  ...
  #### <a name="allow-the-webhid-api-on-these-sites"></a>이러한 사이트에서 WebHID API 허용
...
정책을 설정하지 않으면 [DefaultWebHidGuardSetting](#defaultwebhidguardsetting)이 설정된 경우 모든 사이트에 적용됩니다. 그렇지 않은 경우에는 사용자의 개인 설정이 적용됩니다.

정책과 일치하지 않는 URL 패턴의 경우 다음 값이 우선 순위에 따라 적용됩니다.

  * [WebHidBlockedForUrls](#webhidblockedforurls) (일치하는 항목이 있는 경우),

  * [DefaultWebHidGuardSetting](#defaultwebhidguardsetting) (설정된 경우), 또는

  * 사용자의 개인 설정

URL 패턴은 [WebHidBlockedForUrls](#webhidblockedforurls)와 충돌하지 않아야 합니다. URL이 두 패턴 모두 일치하는 경우 두 정책 모두 우선되지 않습니다.
...
```
  ...
  #### <a name="block-the-webhid-api-on-these-sites"></a>이러한 사이트에서 WebHID API 차단
...
정책과 일치하지 않는 URL 패턴의 경우 다음 값이 우선 순위에 따라 적용됩니다.
  ...
  * [WebHidAskForUrls](#webhidaskforurls) (일치하는 항목이 있는 경우),

  * [DefaultWebHidGuardSetting](#defaultwebhidguardsetting) (설정된 경우), 또는

  * 사용자의 개인 설정

URL 패턴은 [WebHidAskForUrls](#webhidaskforurls)와 충돌할 수 없습니다. URL이 두 패턴 모두 일치하는 경우 두 정책 모두 우선되지 않습니다.
...
```
  ...
  ## <a name="experimentation-policies"></a>실험 정책

  [맨 위로 이동](#microsoft-edge---policies)

  ### <a name="featureflagoverridescontrol"></a>FeatureFlagOverridesControl

  #### <a name="configure-users-ability-to-override-feature-flags"></a>기능 플래그를 재정의하는 사용자 기능 구성
  ...
  기능 플래그의 상태를 재정의하는 사용자 기능을 구성합니다.
이 정책을 'CommandLineOverridesEnabled'로 설정하면 사용자가 edge://flags 페이지가 아닌 명령줄 인수를 사용하여 기능 플래그의 상태를 재정의할 수 있습니다.
...
```
  ...
  #### <a name="configure-default-state-of-allow-extensions-from-other-stores-setting"></a>다른 스토어에서 확장 허용 설정의 기본 상태 구성
  ...
  이 정책을 통해 다른 저장소에서 확장 허용 설정의 기본 상태를 제어할 수 있습니다.
이 정책을 사용하여 Chrome 웹 스토어와 같은 다른 저장소의 확장 설치를 중지할 수 없습니다.
다른 저장소의 확장 설치를 중지하려면 확장 설정 정책 [https://go.microsoft.com/fwlink/?linkid=2187098](https://go.microsoft.com/fwlink/?linkid=2187098)을(를) 사용하세요.

사용하도록 설정하면 다른 스토어의 확장 허용이 켜집니다. 따라서 사용자는 Chrome 웹 스토어와 같이 지원되는 다른 스토어에서 확장을 설치하는 동안 플래그를 수동으로 켤 필요가 없습니다. 그러나 사용자는 이 설정을 재정의할 수 있습니다.
사용자가 이미 설정을 실행했다가 실행 중지한 경우 이 설정이 작동하지 않을 수 있습니다.
관리자가 먼저 정책을 사용으로 설정한 다음 구성되지 않거나 사용 안 함으로 변경하면 사용자 설정에 영향을 주지 않고 설정은 그대로 유지됩니다.
...
해당 정책은 확장 ID 또는 업데이트 URL을 해당 특정 설정에만 매핑합니다. 이 정책에서 사용자 지정 구성이 없는 모든 확장에 적용되는 특수 ID "*"에 대해 기본 구성을 설정할 수 있습니다. 업데이트 URL을 사용할 경우 확장 매니페스트에 설명된 정확한 업데이트 URL이 있는 확장에 구성이 적용됩니다. 'override_update_url' 플래그가 true로 설정되면 [ExtensionInstallForcelist](#extensioninstallforcelist) 정책 또는 이 정책의 'update_url' 필드에 지정된 업데이트 URL을 사용하여 확장 프로그램이 설치되고 업데이트됩니다. 'update_url'이 Edge 추가 기능 웹사이트 업데이트 URL인 경우 'override_update_url' 플래그는 무시됩니다. 자세한 내용은 [https://go.microsoft.com/fwlink/?linkid=2161555](https://go.microsoft.com/fwlink/?linkid=2161555)에서 사용 가능한 ExtensionSettings 정책에 대한 자세한 가이드를 참조하세요.
...
```
  ...
  #### <a name="list-of-origins-that-allow-all-http-authentication"></a>모든 HTTP 인증을 허용하는 원본 목록
  ...
  이 정책을 설정하여 [AuthSchemes](#authschemes) 정책에 관계없이 Microsoft Edge에서 지원하는 모든 HTTP 인증 체계를 허용하는 원본을 지정합니다.

이 형식에 따라 원본 패턴의 서식을 지정합니다(https://www.chromium.org/administrators/url-blocklist-filter-format)). [AllHttpAuthSchemesAllowedForOrigins](#allhttpauthschemesallowedfororigins)에 최대 1,000개의 예외를 정의할 수 있습니다.
전체 원본 또는 원본 부분에 와일드카드를 사용할 수 있습니다. 파트에는 체계, 호스트 또는 포트가 포함됩니다.
...
```
  ...
  ## <a name="identity-and-sign-in-policies"></a>ID 및 로그인 정책

  [맨 위로 이동](#microsoft-edge---policies)

  ### <a name="edgedefaultprofileenabled"></a>EdgeDefaultProfileEnabled

  #### <a name="default-profile-setting-enabled"></a>기본 프로필 설정 사용
  ...
  이 정책을 구성하면 사용된 마지막 프로필이 아닌 브라우저를 열 때 사용할 기본 프로필을 Microsoft Edge에서 설정할 수 있습니다. "--profile-directory" 매개 변수가 지정되어 있을 경우 이 정책은 영향을 주지 않습니다. 기본 프로필을 나타내려면 값을 "Default"로 설정합니다. 이 값은 대/소문자를 구분합니다.
정책 값은 프로필의 이름이며(대/소문자 구분) 특정 프로필의 이름인 문자열로 구성할 수 있습니다.
"Edge Kids Mode" 및 "Guest Profile" 값은 기본 프로필이 아니므로 유용하지 않은 것으로 간주됩니다.
이 정책은 다음 시나리오에 영향을 주지 않습니다.
  1) “프로필 기본 설정”의 “사이트에 대한 프로필 기본 설정”에 지정된 설정
  2) Outlook 및 Teams에서 열리는 링크입니다.

다음 문은 "--profile-directory"를 지정하지 않는 조건이며 구성된 값이 "Edge Kids Mode" 또는 "Guest Profile"이 아닙니다. 이 정책을 사용하도록 설정하고 특정 프로필 이름으로 구성하며 지정된 프로필을 찾을 수 없으면 Microsoft Edge는 시작 시 지정된 프로필을 사용하고 “외부 링크의 기본 프로필” 설정이 지정된 프로필로 변경되고 회색으로 표시됩니다. 이 정책을 사용하도록 설정하고 특정 프로필 이름으로 구성하지만 찾을 수 없는 경우 정책은 이전에 설정되지 않은 것처럼 동작합니다.
이 정책을 사용하도록 설정하지만 구성하거나 사용하지 않도록 설정하지 않으면 정책은 이전에 설정된 적이 없는 것처럼 동작합니다.
...
```
  ...
  #### <a name="enable-implicit-sign-in"></a>암시적 로그인 사용
...
```
  ...
  #### <a name="oneauth-authentication-flow-enforced-for-signin"></a>로그인을 위해 적용된 OneAuth 인증 흐름
...
이 정책을 사용하지 않거나 구성하지 않으면 로그인 프로세스에서 Windows 계정 관리자를 사용합니다. Microsoft Edge는 암호 없이 사용자가 Windows, Microsoft Office 또는 기타 Microsoft 응용 프로그램에 로그인한 계정을 사용하여 로그인할 수 있습니다. 또는 로그인을 위해 유효한 계정과 암호를 제공할 수 있습니다. 이 암호는 나중에 사용할 수 있도록 Windows 계정 관리자에 저장됩니다. Windows 설정 -> 계정 -> 이메일 및 계정 페이지를 통해 Windows 계정 관리자에 저장된 모든 계정을 조사할 수 있습니다.
...
```
  ...
  #### <a name="only-on-premises-account-enabled-for-implicit-sign-in"></a>암시적 로그인에 대해 온-프레미스 계정만 사용하도록 설정됨
...
이 정책은 정책 [ConfigureOnPremisesAccountAutoSignIn](#configureonpremisesaccountautosignin)이 사용하도록 설정되고 'SignInAndMakeDomainAccountNonRemovable'로 설정된 경우에만 적용됩니다.
...
```
  ...
  #### <a name="enable-sign-in-click-to-action-dialog"></a>로그인 클릭 투 액션 대화 상자 사용
...
```
  ...
  #### <a name="wam-for-authentication-below-windows-10-rs3-enabled"></a>아래 인증을 위한 WAM Windows 10 RS3 사용 설정됨
...
```
  ...
  #### <a name="swipe-gestures-in-microsoft-edge-kiosk-mode-enabled"></a>Microsoft Edge 키오스크 모드에서 살짝 밀기 제스처 사용
...
키오스크 모드를 구성하는 방법에 대한 자세한 내용은 [https://go.microsoft.com/fwlink/?linkid=2137578](https://go.microsoft.com/fwlink/?linkid=2137578)을(를) 참조하세요.
...
```
  ...
  #### <a name="allow-users-to-get-a-strong-password-suggestion-whenever-they-are-creating-an-account-online"></a>사용자가 온라인으로 계정을 만들 때마다 강력한 암호 제안을 받을 수 있도록 허용
...
```
  ...
  #### <a name="configure-the-list-of-domains-for-which-the-password-manager-ui-save-and-fill-will-be-disabled"></a>암호 관리자 UI(저장 및 채우기)가 비활성화될 도메인 목록을 구성합니다.
...
```
  ...
  #### <a name="configures-a-setting-that-asks-users-to-enter-their-device-password-while-using-password-autofill"></a>암호 자동 채우기를 사용하는 동안 사용자에게 디바이스 암호를 입력하도록 요청하는 설정 구성
...
이 정책을 '자동으로, 이 정책을 사용하지 않도록 설정하거나 이 정책을 구성하지 않음’으로 설정하면 자동 완성에 인증 흐름이 없게 됩니다.

이 정책을 '장치 암호 사용'으로 설정하면 사용자가 장치 암호(또는 Windows의 경우 Windows Hello의 기본 인증 모드 - Mac의 경우 PIN, 얼굴 인식 또는 지문 및 이에 상응하는 옵션)를 입력하여 신원을 증명해야 하고, 이 경우에만 암호가 자동으로 채워집니다. 또한 인증 프롬프트 빈도는 기본적으로 '항상'으로 설정되지만 사용자는 다른 옵션인 '검색 세션마다 한 번'으로 변경할 수도 있습니다.
...
```
  ...
  #### <a name="print-postscript-mode"></a>PostScript 모드 인쇄
...
```
  ...
  #### <a name="print-rasterize-pdf-dpi"></a>PDF DPI 래스터화 인쇄
...
```
  ...
  #### <a name="sets-layout-for-printing"></a>인쇄를 위한 레이아웃 설정
...
이 정책을 사용하지 않도록 설정하거나 구성하지 않으면 사용자는 세로 또는 가로 레이아웃으로 웹 페이지를 인쇄할지 여부를 결정할 수 있습니다.
...
```
  ...
  ## <a name="private-network-request-settings-policies"></a>개인 네트워크 요청 설정 정책

  [맨 위로 이동](#microsoft-edge---policies)

  ### <a name="insecureprivatenetworkrequestsallowed"></a>InsecurePrivateNetworkRequestsAllowed

  #### <a name="specifies-whether-to-allow-insecure-websites-to-make-requests-to-more-private-network-endpoints"></a>안전하지 않은 웹사이트가 더 많은 개인 네트워크 엔드포인트에 요청을 할 수 있도록 허용할지 여부를 지정
  ...
  안전하지 않은 웹 사이트가 더 많은 사설 네트워크 엔드포인트에 요청을 할 수 있는지 여부를 제어
...
1) 해당 IP 주소가 localhost이고 다른 주소는 localhost가 아닌 경우
...
해당 정책을 true로 설정하면 안전하지 않은 웹 사이트에서 다른 교차 원본 검사에 따라 모든 네트워크 엔드포인트에 요청을 할 수 있습니다.
  ...
  - GP 이름: 안전하지 않은 웹사이트가 더 많은 개인 네트워크 엔드포인트에 요청을 할 수 있도록 허용할지 여부를 지정
...
```
  ...
  #### <a name="allow-the-listed-sites-to-make-requests-to-more-private-network-endpoints-from-insecure-contexts"></a>나열된 사이트가 안전하지 않은 컨텍스트에서 더 많은 개인 네트워크 엔드포인트에 요청을 할 수 있도록 허용
  ...
  URL 패턴 목록. 일치하는 원본에서 제공되는 안전하지 않은 웹사이트에서 시작된 개인 네트워크 요청이 허용됩니다.

해당 정책을 설정하지 않은 경우 이 정책은 빈 목록으로 설정된 경우처럼 작동합니다.

여기서 지정한 패턴으로 다루지 않는 원본의 경우 전역 기본값은 [InsecurePrivateNetworkRequestsAllowed](#insecureprivatenetworkrequestsallowed) 정책(설정된 경우) 또는 사용자의 개인 구성에서 사용됩니다.
...
유효한 URL 패턴에 대한 자세한 내용은 [URL 목록 기반 정책의 필터 형식](/DeployEdge/edge-learnmmore-url-list-filter%20format)을 참조하세요.
  ...
  - GP 이름: 나열된 사이트가 안전하지 않은 컨텍스트에서 더 많은 개인 네트워크 엔드포인트에 요청을 할 수 있도록 허용
  - GP경로(필수): Administrative Templates/Microsoft Edge/Private Network Request Settings
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
...
해당 정책은 [ProxySettings](#proxysettings) 정책이 지정되지 않고 사용자가 [ProxyMode](#proxymode) 정책에서 fixed_servers 또는 pac_script를 선택한 경우에만 적용됩니다. 프록시 정책을 구성하기 위해 다른 모드를 선택한 경우 해당 정책을 사용하지 않도록 설정하거나 구성하지 않아야 합니다.
...
이 정책을 사용하면 홈 버튼이 사용자가 구성한 새 탭 페이지로 설정되거나 [NewTabPageLocation](#newtabpagelocation) 정책으로 설정되고 [HomepageLocation](#homepagelocation)으로 설정된 URL이 고려되지 않습니다.

이 정책을 비활성화하면 홈 버튼은 사용자가 구성하거나 [HomepageLocation](#homepagelocation) 정책에 구성된 URL을 설정합니다.
...
여기에서 URL을 설정하거나 홈 페이지를 설정하여 새 탭 페이지 'edge://newtab'을 열 수 있습니다. 기본적으로 홈 버튼은 새 탭 페이지를 열고(사용자가 구성하거나 [NewTabPageLocation](#newtabpagelocation) 정책으로 구성) 사용자는 이 정책과 새 탭 페이지에서 구성한 URL 중에서 선택할 수 있습니다.

이 정책을 사용하도록 설정하면 사용자는 홈 페이지 URL을 변경할 수 없지만 홈 버튼의 동작을 선택하여 설정된 URL 또는 새 탭 페이지를 열 수 있습니다. 설정된 URL을 사용하려면 [HomepageIsNewTabPage](#homepageisnewtabpage)=Disabled도 구성해야 합니다.
...
```
  ...
  #### <a name="allow-microsoft-news-content-on-the-new-tab-page"></a>새 탭 페이지에서 Microsoft 뉴스 콘텐츠 허용
...
관련 정책: [NewTabPageAllowedBackgroundTypes](#newtabpageallowedbackgroundtypes), [NewTabPageQuickLinksEnabled](#newtabpagequicklinksenabled)
...
관련 정책: [NewTabPageAllowedBackgroundTypes](#newtabpageallowedbackgroundtypes), [NewTabPageContentEnabled](#newtabpagecontentenabled)
...
```
  ...
  #### <a name="configure-the-microsoft-edge-new-tab-page-experience-obsolete"></a>Microsoft Edge 새 탭 페이지 환경 구성(사용되지 않음)
...
```
  ...
  #### <a name="single-sign-on-for-work-or-school-sites-using-this-profile-enabled"></a>이 프로필을 사용하는 직장 또는 학교 사이트에 대한 Single Sign-On 설정됨
...
```
  ...
  #### <a name="let-screen-reader-users-get-image-descriptions-from-microsoft"></a>화면 읽기 프로그램 사용자가 Microsoft에서 이미지 설명을 가져올 수 있도록 허용
...
```
  ...
  #### <a name="configure-if-the-ads-transparency-feature-is-enabled"></a>광고 투명도 기능을 사용할 수 있는지 구성
  ...
  광고 투명성 기능을 사용할지 여부를 결정할 수 있습니다. 이 동작은 추적 방지의 "균형" 모드에만 적용되며 "기본" 또는 "엄격한" 모드에는 영향을 주지 않습니다. 사용자의 추적 방지 수준은 [TrackingPrevention](#trackingprevention) 정책을 사용하여 구성할 수 있습니다. AdsTransparencyEnabled는 [TrackingPrevention](#trackingprevention)이 TrackingPreventionBalanced로 설정되었거나 구성되지 않은 경우에만 영향을 줍니다.
...
```
  ...
  #### <a name="allow-users-to-access-the-games-menu"></a>사용자가 게임 메뉴에 액세스할 수 있도록 허용
...
```
  ...
  #### <a name="allow-pages-to-send-synchronous-xhr-requests-during-page-dismissal-obsolete"></a>페이지를 해제하는 동안 페이지에서 동기 XHR 요청 전송 허용(사용하지 않음)
...
```
  ...
  #### <a name="allows-the-appcache-feature-to-be-re-enabled-even-if-its-turned-off-by-default-obsolete"></a>AppCache 기능이 기본적으로 해제되어 있더라도 다시 사용할 수 있도록 허용(사용하지 않음)
...
```
  ...
  #### <a name="allow-the-audio-process-to-run-with-priority-above-normal-on-windows"></a>Windows에서 오디오 프로세스가 정상보다 우선 순위로 실행되도록 허용
  ...
  이 정책은 Windows에서 오디오 프로세스의 우선 순위를 제어합니다.
이 정책을 사용하도록 설정하면 오디오 프로세스가 일번 우선 순위 이상으로 실행됩니다.
이 정책을 사용하지 않도록 설정하면 오디오 프로세스가 일반 우선 순위로 실행됩니다.
해당 정책을 구성하지 않은 경우 오디오 프로세스의 기본 구성이 사용됩니다.
이 정책은 엔터프라이즈에서 오디오 캡처를 통해 특정 성능 문제를 해결하기 위해 높은 우선순위로 오디오를 실행할 수 있도 임시 측정값으로 고안된 것입니다.
해당 정책은 추후 제거됩니다.
...
```
  ...
  #### <a name="enable-autofill-for-payment-instruments"></a>결제 방법에 자동 채우기 사용
...
```
  ...
  #### <a name="configure-automatic-https"></a>자동 HTTPS 구성
  ...
  이 정책을 사용하면 HTTP에서 HTTPS로 연결을 전환하는 [AutomaticHttpsDefault](#automatichttpsdefault)의 설정을 관리할 수 있습니다.

이 기능은 더 안전한 연결을 적용하여 중간자 공격을 방지하지만 사용자가 더 많은 연결 오류를 경험할 수 있습니다.

참고: 'UpgradeCapableDomains' 구성에는 구성 요소 목록이 필요하며 [ComponentUpdatesEnabled](#componentupdatesenabled)가 'Disabled'로 설정된 경우 이러한 연결을 업그레이드하지 않습니다.

이 정책을 구성하지 않는 경우 [AutomaticHttpsDefault](#automatichttpsdefault)가 사용하도록 설정되고 HTTPS를 지원할 가능성이 높은 도메인의 연결만 업그레이드합니다.

정책 옵션 매핑:

* DisableAutomaticHttps (0) = 자동 HTTPS 기능이 사용되지 않습니다.

* UpgradeCapableDomains (1) = HTTP를 통해 전달된 탐색은 HTTPS를 지원할 가능성이 높은 도메인에서만 HTTPS로 전환됩니다.
...
```
  ...
  #### <a name="allow-media-autoplay-on-specific-sites"></a>특정 사이트에서 미디어 자동 실행 허용
...
해당 정책을 구성하지 않으면 [AutoplayAllowed](#autoplayallowed) 정책(설정된 경우) 또는 사용자의 개인 구성의 전역 기본 값이 모든 사이트에 대해 사용됩니다.
...
```
  ...
  #### <a name="enables-background-updates-to-the-list-of-available-templates-for-collections-and-other-features-that-use-templates-deprecated"></a>서식 파일을 사용하는 컬렉션 및 기타 기능에 대해 사용 가능한 서식 파일 목록의 백그라운드 업데이트 사용(사용되지 않음)
  ...
  이 정책은 새 정책으로 이동 중이므로 더 이상 사용되지 않습니다. 버전 104부터 바로 Microsoft Edge에서 작동하지 않습니다. 사용할 새 정책은 [EdgeAssetDeliveryServiceEnabled](#edgeassetdeliveryserviceenabled)입니다.
...
```
  ...
  #### <a name="enable-browser-legacy-extension-point-blocking"></a>브라우저 레거시 확장 지점 차단 사용
...
Microsoft Edge는 'time_to_live_in_hours'보다 오래된 선택한 유형의 데이터를 정기적으로 제거합니다. 만료된 데이터의 삭제는 브라우저가 시작된 후 15초 후에 브라우저가 실행되는 동안 매 시간마다 발생합니다.
...
Mac OS X에서 레거시 인증서 검증 도구에 대한 지원이 제거될 예정인 경우 Microsoft Edge 버전 107에서 작동하지 않습니다.
...
```
  ...
  #### <a name="cecpq2-post-quantum-key-agreement-enabled-for-tls"></a>TLS에 대해 CECPQ2 post-quantum key 계약 설정됨
...
```
  ...
  #### <a name="configure-the-list-of-commands-for-which-to-disable-keyboard-shortcuts"></a>바로 가기 키를 사용하지 않도록 설정할 명령 목록 구성
...
사용하지 않도록 설정할 수 있는 명령 목록은 [https://go.microsoft.com/fwlink/?linkid=2186950](https://go.microsoft.com/fwlink/?linkid=2186950)을(를) 참조하세요.
...
Microsoft Edge 89 이후부터 [RoamingProfileSupportEnabled](#roamingprofilesupportenabled) 정책이 사용하지 않도록 설정된 기존 온-프레미스 프로필이 있는 경우 이제 컴퓨터는 하이브리드에 가입된 것입니다. Azure AD 계정이 있는 경우, 전체 Azure AD 동기화 기능을 얻을 수 있도록 온-프레미스 프로필을 Azure AD 프로필로 자동 업그레이드합니다.

Microsoft Edge 93부터는 정책 [ImplicitSignInEnabled](#implicitsigninenabled)를 사용하지 않도록 설정하면 이 정책이 적용되지 않습니다.

Microsoft Edge 94부터 정책 [OnlyOnPremisesImplicitSigninEnabled](#onlyonpremisesimplicitsigninenabled)를 사용하도록 설정하고 이 정책이 'SignInAndMakeDomainAccountNonRemovable'로 설정된 경우 하이브리드 조인된 환경에서도 적용됩니다. Microsoft Edge는 MSA 또는 AAD 계정이 있더라도 Active Directory 도메인 계정을 사용하여 사용자를 자동으로 로그인합니다.
...
이 기능은 파일 탐색기에서 보기 옵션을 사용하는 대신 Microsoft Edge를 통해 사용할 수 있는 옵션이지만 SharePoint 외부에서 파일 및 폴더를 관리하는 데 권장되는 방법은 SharePoint 파일을 동기화하거나 SharePoint에서 파일을 이동 또는 복사하는 것입니다.
SharePoint 파일 동기화: https://go.microsoft.com/fwlink/p/?linkid=2166983 SharePoint에서 파일 제거 또는 복사: https://go.microsoft.com/fwlink/p/?linkid=2167123
...
```
  ...
  #### <a name="specifies-whether-webassembly-modules-can-be-sent-cross-origin-obsolete"></a>WebAssembly 모듈을 원본 간으로 보낼 수 있는지 여부를 지정(사용되지 않음)
...
```
  ...
  #### <a name="specifies-whether-the-display-capture-permissions-policy-is-checked-or-skipped"></a>디스플레이 캡처 권한 정책을 선택하거나 건너뛸지 여부를 지정합니다.
  ...
  이 사양에 따른 getDisplayMedia()에 대한 표시 캡처 권한-정책 게이트 액세스: https://www.w3.org/TR/screen-capture/#feature-policy-integration 그러나 이 정책이 사용 안 함으로 설정되어 있으면 이 요구 사항이 적용되지 않고 금지된 컨텍스트에서 getDisplayMedia()가 허용됩니다. 이 Enterprise 정책은 일시적입니다. Microsoft Edge 버전 100 이후에 제거 될 예정입니다.
애플리케이션이 사양을 준수하지 않지만 수정할 시간이 필요한 Enterprise 사용자를 차단 해제하기 위한 정책입니다.

해당 정책을 사용하도록 설정하거나 구성하지 않는 경우 사이트는 디스플레이 캡처 권한-정책에 의해 허용되는 컨텍스트에서만 getDisplayMedia()를 호출할 수 있습니다.

해당 정책을 사용하지 않도록 설정하면 사이트에서 디스플레이 캡처 권한 정책에 의해 허용되지 않는 컨텍스트에서도 getDisplayMedia()를 호출할 수 있습니다.
다른 제한은 계속 적용될 수 있습니다.
  ...
  - GP 이름: 디스플레이 캡처 사용 권한 정책을 선택하거나 건너뜁니다.
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
...
```
  ...
  #### <a name="define-a-list-of-protocols-that-can-not-be-silently-blocked-by-anti-flood-protection"></a>플러드 방지 보호로 자동 차단할 수 없는 프로토콜 목록 정의
...
알려진 다운로드 또는 잠재적으로 위험한 다운로드에 대한 Microsoft Defender SmartScreen 경고를 전달하거나 위험한 파일 형식 확장명이 있는 다운로드를 제외한 모든 다운로드를 허용하도록 'BlockDangerousDownloads'를 설정합니다.
...
```
  ...
  #### <a name="allow-features-to-download-assets-from-the-asset-delivery-service"></a>Asset Delivery Service에서 자산을 다운로드하는 기능 허용
  ...
  Asset Delivery Service는 Microsoft Edge 클라이언트에 자산을 배달하는 데 사용되는 일반 파이프라인입니다.
이러한 자산은 이 서비스를 사용하는 기능을 지원하는 구성 파일 또는 기계 학습 모델이 될 수 있습니다.
...
```
  ...
  #### <a name="allow-certificates-signed-using-sha-1-when-issued-by-local-trust-anchors-obsolete"></a>로컬 트러스트 앵커에서 발급한 경우 SHA-1을 사용하여 서명한 인증서 허용(사용되지 않음)
...
```
  ...
  #### <a name="explicitly-allowed-network-ports"></a>명시적으로 허용된 네트워크 포트
...
```
  ...
  #### <a name="configure-whether-microsoft-edge-should-automatically-select-a-certificate-when-there-are-multiple-certificate-matches-for-a-site-configured-with-autoselectcertificateforurls-deprecated"></a>"AutoSelectCertificateForUrls"를 사용하여 구성한 사이트에 대해 일치하는 인증서가 여러 개인 경우 Microsoft Edge에서 자동으로 인증서를 선택할 것인지 여부 지정(사용되지 않음)
  ...
  이 정책은 새 정책으로 이동 중이므로 더 이상 사용되지 않습니다. Microsoft Edge 버전 104에서는 작동하지 않습니다. 사용할 새 정책은 [PromptOnMultipleMatchingCertificates](#promptonmultiplematchingcertificates)입니다.
...
```
  ...
  #### <a name="enable-or-disable-freezing-the-user-agent-string-at-major-version-99"></a>주 버전 99에서 사용자 에이전트 문자열 고정 사용 또는 사용 안 함
...
웹 사이트에서 사용자-에이전트 요청 헤더를 사용하여 응용 프로그램, 운영 체제, 공급업체 및/또는 요청하는 사용자 에이전트의 버전을 식별할 수 있습니다.
일부 웹 사이트에서는 이 헤더의 형식을 가정하고 주 위치에 세 자리 숫자가 포함된 버전 문자열(예: 100.0.0.0)에 문제가 발생할 수 있습니다.

정책을 '기본값'으로 설정하거나 설정을 해제한 상태로 두면 사용자-에이전트 문자열 주 버전에 대한 브라우저 설정이 기본적으로 사용됩니다.
'ForceEnabled'로 설정하면 사용자-에이전트 문자열은 항상 주 버전을 99로 보고하고 브라우저의 주 버전을 보조 위치에 포함합니다. 예를 들어 브라우저 버전 101.0.0.0은 버전 99.101.0.0을 보고하는 사용자-에이전트 요청 헤더를 보냅니다.
'ForceDisabled'로 설정하면 사용자-에이전트 문자열이 주 버전을 고정하지 않습니다.
...
해당 정책에 대한 다음 데이터 형식 중 하나를 제공할 수 있습니다. "favorites", "settings", "passwords", "addressesAndMore", "extensions", "history", "openTabs" 및 “collections” "앱" 데이터 형식은 Microsoft Edge 버전 100부터 지원됩니다. 해당 데이터 형식 이름은 대/소문자를 구분해야 한다는 점에 유의하세요.
...
```
  ...
  #### <a name="control-use-of-the-headless-mode"></a>비입력 시스템 모드 사용 제어
  ...
  이 정책 설정을 사용하면 사용자가 비입력 시스템 모드에서 Microsoft Edge를 시작할 수 있는지 여부를 결정할 수 있습니다.

이 정책을 사용하도록 설정하거나 구성하지 않으면 Microsoft Edge에서 비입력 시스템 모드를 사용할 수 있습니다.

이 정책을 사용하지 않도록 설정하면 Microsoft Edge에서 비입력 시스템 모드를 사용할 수 없습니다.
  ...
  - GP 고유 이름: HeadlessModeEnabled
  - GP 이름: 비입력 시스템 모드 사용 제어
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
...
```
  ...
  - 기본 설정 키 이름: HeadlessModeEnabled
  - 예제 값:
``` xml
<true/>
```
...
```
  ...
  #### <a name="hide-restore-pages-dialog-after-browser-crash"></a>브라우저 작동 중단 후 복원 페이지 대화 상자 숨기기
  ...
  이 정책은 Microsoft Edge가 충돌한 후 "페이지 복원" 대화 상자를 숨길 수 있는 옵션을 제공합니다. "페이지 복원" 대화 상자는 Microsoft Edge가 충돌하기 전에 열려 있던 페이지를 복원할 수 있는 옵션을 제공합니다.

이 정책을 사용하도록 설정하면 "페이지 복원" 대화 상자가 표시되지 않습니다.

이 정책을 사용하지 않도록 설정하거나 설정하지 않으면 "페이지 복원" 대화 상자가 표시됩니다.

이 정책을 설정한 경우 기록이 저장되지 않아 대화 상자도 사용할 수 없으므로 [ClearBrowsingDataOnExit](#clearbrowsingdataonexit) 또는 [SavingBrowserHistoryDisabled](#savingbrowserhistorydisabled) 정책을 설정하지 마세요.
  ...
  - GP 고유 이름: HideRestoreDialogEnabled
  - GP 이름: 브라우저 작동 중단 후 복원 페이지 대화 상자 숨기기
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
...
```
  ...
  - 기본 설정 키 이름: HideRestoreDialogEnabled
  - 예를 들어 값:
``` xml
<false/>
```
  ...
  #### <a name="show-hubs-sidebar"></a>허브 사이드바 표시
  ...
  Microsoft Edge 화면 오른쪽에 시작 관리자 막대를 표시합니다.

해당 정책을 사용하면 항상 사이드바가 표시됩니다.
사이드바를 표시하지 않으려면 이 정책을 사용하지 않도록 설정합니다.

해당 정책을 구성하지 않으면 사용자는 사이드바를 표시할지 여부를 선택할 수 있습니다.
...
해당 정책을 권장 사항으로 설정할 수 있습니다. 즉, Microsoft Edge가 처음 실행 시 확장을 가져오지만 사용자가 수동으로 가져오는 동안 **확장** 옵션을 선택하거나 선택 취소할 수 있습니다.
...
```
  ...
  #### <a name="allow-importing-of-startup-page-settings"></a>시작 페이지 설정 가져오기 허용
  ...
  사용자가 다른 브라우저에서 Microsoft Edge로 시작 설정을 가져올 수 있도록 허용

이 정책을 사용하도록 설정하면 시작 설정을 항상 가져오게 됩니다.

이 정책을 사용하지 않도록 설정하면 처음 실행 시 또는 수동 가져오기 시 시작 설정을 가져오지 않습니다.

이 정책을 구성하지 않은 경우 처음 실행 시 시작 설정을 가져오며 사용자는 나중에 탐색 세션에서 브라우저 설정 옵션을 선택하여 이 데이터를 수동으로 가져올지 여부를 선택할 수 있습니다.
...
**참고**: 이 정책은 현재 Microsoft Edge 레거시 및 Google Chrome(Windows 7, 8 및 10) 브라우저에서 가져오기를 관리합니다.
  ...
  - GP 고유 이름: ImportStartupPageSettings
  - GP 이름: 시작 페이지 설정 가져오기 허용
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 관리 템플릿/Microsoft Edge - 기본 설정(사용자 재정의 가능)/
  - GP ADMX 파일 이름: MSEdge.admx
...
```
  ...
  #### <a name="configure-reporting-of-potentially-misconfigured-neutral-site-urls-to-the-m365-admin-center-site-lists-app"></a>잠재적으로 잘못 구성된 중립 사이트 URL을 M365 관리 센터 사이트 목록 앱에 보고 구성
  ...
  이 설정을 사용하면 엔터프라이즈 모드 사이트 목록에서 중립 사이트로 구성해야 하는 사이트에 대한 보고를 실행할 수 있습니다. 보고서를 보내려면 사용자가 올바른 직장 또는 학교 계정으로 Microsoft Edge에 로그인해야 하며 사용자의 계정 테넌트가 정책에서 지정한 테넌트와 일치해야 합니다.

이 정책을 구성하는 경우 Microsoft Edge와 Internet Explorer 엔진 간에 여러 번 리디렉션하는 동안 탐색이 고착된 것으로 나타나면 Microsoft Edge에서 M365 관리 센터 사이트 목록 앱으로 보고서를 보냅니다. 일반적으로 인증 서버에 대한 리디렉션이 전환 엔진이며, 루프에서 반복적으로 실패함을 나타냅니다. 보고서는 리디렉션 대상인 사이트의 URL에서 쿼리 문자열 또는 조각을 뺀 값을 표시합니다. 사용자의 ID가 보고되지 않습니다.

이 보고가 올바르게 작동하려면 M365 관리 센터에서 Microsoft Edge 사이트 목록 앱을 한 번 이상 방문해야 합니다. 이렇게 하면 이러한 보고서를 저장하는 데 사용되는 테넌트별 저장소 계정이 활성화됩니다. Microsoft Edge는 이 단계가 완료되지 않은 경우에도 보고서를 보내려고 시도합니다. 그러나 보고서는 사이트 목록 앱에 저장되지 않습니다.

이 정책을 사용하도록 설정하는 경우 O365 테넌트 ID를 지정해야 합니다. O365 테넌트 ID를 찾는 방법에 대한 자세한 내용은 [https://go.microsoft.com/fwlink/?linkid=2185668](https://go.microsoft.com/fwlink/?linkid=2185668)을(를) 참조하세요.

이 정책을 사용하지 않도록 설정하거나 구성하지 않으면 Microsoft Edge는 잠재적으로 잘못 구성된 중립 사이트에 대한 보고서를 사이트 목록 앱으로 보내지 않습니다.

Internet Explorer 모드에 대한 자세한 내용은 [https://go.microsoft.com/fwlink/?linkid=2165707](https://go.microsoft.com/fwlink/?linkid=2165707)을 참조하세요.
  ...
  - GP 고유 이름: InternetExplorerIntegrationCloudNeutralSitesReporting
  - GP 이름: 잠재적으로 잘못 구성된 중립 사이트 URL을 M365 관리 센터 사이트 목록 앱에 보고 구성
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
  ...
  - 경로 (필수): SOFTWARE\정책\Microsoft\Edge
  - 경로(권장): 해당 없음
  - 값 이름: InternetExplorerIntegrationCloudNeutralSitesReporting
  - 값 형식: REG_SZ
...
```
  ...
  #### <a name="configure-the-enterprise-mode-cloud-site-list"></a>엔터프라이즈 모드 클라우드 사이트 목록 구성
  ...
  M365 관리 센터의 Microsoft Edge 사이트 목록 설정을 사용하면 규격 클라우드 위치에서 사이트 목록을 호스트하고 기본 제공 환경을 통해 사이트 목록의 콘텐츠를 관리할 수 있습니다. 이 설정을 사용하면 사용자에게 배포할 M365 관리 센터 내의 사이트 목록을 지정할 수 있습니다. 사용자는 유효한 회사 또는 학교 계정으로 Microsoft Edge에 로그인해야 합니다. 그렇지 않으면 Microsoft Edge가 클라우드 위치에서 사이트 목록을 다운로드하지 않습니다.

이 설정은 [InternetExplorerIntegrationLevel](#internetexplorerintegrationlevel) 설정이 구성된 경우에만 적용됩니다.

이 정책을 구성하면 Microsoft Edge가 지정된 사이트 목록을 사용합니다. 사용하도록 설정하면 M365 관리 센터에서 만들고 클라우드에 게시한 사이트 목록의 식별자를 입력할 수 있습니다.

이 설정은 Microsoft Edge의 [InternetExplorerIntegrationSiteList](#internetexplorerintegrationsitelist) 정책 및 Internet Explorer 사이트 목록 설정(엔터프라이즈 모드 IE 웹 사이트 목록 사용)보다 우선합니다. 이 정책을 사용하지 않도록 설정하거나 구성하지 않으면 Microsoft Edge가 [InternetExplorerIntegrationSiteList](#internetexplorerintegrationsitelist) 정책을 대신 사용합니다.
...
Internet Explorer 모드에 대한 자세한 내용은 [https://go.microsoft.com/fwlink/?linkid=2165707](https://go.microsoft.com/fwlink/?linkid=2165707)을 참조하세요.
  ...
  - GP 고유 이름: InternetExplorerIntegrationCloudSiteList
  - GP 이름: 엔터프라이즈 모드 클라우드 사이트 목록 구성
  - GP 경로 (필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
  ...
  - 경로 (필수): SOFTWARE\정책\Microsoft\Edge
  - 경로(권장): 해당 없음
  - 값 이름: InternetExplorerIntegrationCloudSiteList
  - 값 형식: REG_SZ
...
```
  ...
  #### <a name="configure-reporting-of-ie-mode-user-list-entries-to-the-m365-admin-center-site-lists-app"></a>M365 관리 센터 사이트 목록 앱에 대한 IE 모드 사용자 목록 항목 보고 구성
  ...
  이 설정을 사용하면 Microsoft Edge 사용자가 로컬 IE 모드 사이트 목록에 추가하는 사이트에 대한 보고를 실행할 수 있습니다. 보고서를 보내려면 사용자가 올바른 직장 또는 학교 계정으로 Microsoft Edge에 로그인해야 하며 사용자의 계정 테넌트가 정책에서 지정한 테넌트와 일치해야 합니다.

이 정책을 구성하는 경우 Microsoft Edge는 사용자가 로컬 IE 모드 사이트 목록에 사이트를 추가할 때 M365 관리 센터 사이트 목록 앱으로 보고서를 보냅니다. 보고서에는 사용자가 추가한 사이트의 URL에서 쿼리 문자열 또는 조각을 뺀 값이 표시됩니다. 사용자의 ID가 보고되지 않습니다.

이 보고가 올바르게 작동하려면 M365 관리 센터에서 Microsoft Edge 사이트 목록 앱을 한 번 이상 방문해야 합니다. 이렇게 하면 이러한 보고서를 저장하는 데 사용되는 테넌트별 저장소 계정이 활성화됩니다. Microsoft Edge는 이 단계가 완료되지 않은 경우에도 보고서를 보내려고 시도합니다. 그러나 보고서는 사이트 목록 앱에 저장되지 않습니다.

이 정책을 사용하도록 설정하는 경우 O365 테넌트 ID를 지정해야 합니다. O365 테넌트 ID를 찾는 방법에 대한 자세한 내용은 [https://go.microsoft.com/fwlink/?linkid=2185668](https://go.microsoft.com/fwlink/?linkid=2185668)을(를) 참조하세요.

이 정책을 사용하지 않도록 설정하거나 구성하지 않으면 Microsoft Edge는 사용자의 로컬 사이트 목록에 추가된 URL에 대한 보고서를 사이트 목록 앱으로 보내지 않습니다.
...
Internet Explorer 모드에 대한 자세한 내용은 [https://go.microsoft.com/fwlink/?linkid=2165707](https://go.microsoft.com/fwlink/?linkid=2165707)을 참조하세요.
  ...
  - GP 고유 이름: InternetExplorerIntegrationCloudUserSitesReporting
  - GP 이름: M365 관리 센터 사이트 목록 앱에 대한 IE 모드 사용자 목록 항목 보고 구성
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
  ...
  - 경로 (필수): SOFTWARE\정책\Microsoft\Edge
  - 경로 (권장): 해당 없음
  - 값 이름: InternetExplorerIntegrationCloudUserSitesReporting
  - 값 형식: REG_SZ
...
```
  ...
  #### <a name="show-context-menu-to-open-a-file-link-in-internet-explorer-mode"></a>Internet Explorer 모드에서 file:// 링크를 여는 상황에 맞는 메뉴 표시
...
Internet Explorer 모드에 대한 자세한 내용은 [https://go.microsoft.com/fwlink/?linkid=2094210](https://go.microsoft.com/fwlink/?linkid=2094210)을 참조하세요.
  ...
  - GP 이름: Internet Explorer 모드에서 file:// 링크를 여는 상황에 맞는 메뉴 표시
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
...
```
  ...
  #### <a name="specify-the-number-of-days-that-a-site-remains-on-the-local-ie-mode-site-list"></a>사이트가 로컬 IE 모드 사이트 목록에 남아있는 일 수 지정
...
이 정책을 사용하도록 설정하면 Microsoft Edge 사용자의 로컬 사이트 목록에 사이트가 보존되는 일 수를 입력해야 합니다. 0일에서 90일까지의 값을 사용할 수 있습니다.
...
Internet Explorer 모드에 대한 자세한 내용은 [https://go.microsoft.com/fwlink/?linkid=2094210](https://go.microsoft.com/fwlink/?linkid=2094210)을 참조하세요.
  ...
  - GP 고유 이름: InternetExplorerIntegrationLocalSiteListExpirationDays
  - GP 이름: 사이트가 로컬 IE 모드 사이트 목록에 남아있는 일 수 지정
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
  ...
  - 경로 (필수): SOFTWARE\정책\Microsoft\Edge
  - 경로(권장): 해당 없음
  - 값 이름: InternetExplorerIntegrationLocalSiteListExpirationDays
  - 값 형식: REG_DWORD
...
```
  ...
  #### <a name="allow-unconfigured-sites-to-be-reloaded-in-internet-explorer-mode"></a>구성되지 않은 사이트를 Internet Explorer 모드로 다시 로드할 수 있도록 허용
  ...
  이 정책을 사용하면 사용자가 Microsoft Edge에서 검색할 때 Internet Explorer 모드에서 구성되지 않은 사이트(엔터프라이즈 모드 사이트 목록에 구성되지 않음)를 다시 로드할 수 있으며, 호환성을 위해 사이트에 Internet Explorer가 필요합니다.

사이트가 Internet Explorer 모드로 다시 로드된 후 "페이지 내" 탐색은 Internet Explorer 모드(예: 페이지의 링크, 스크립트 또는 양식 또는 다른 "페이지 내" 탐색에서 서버 쪽 리디렉션)로 유지됩니다. 사용자는 Internet Explorer 모드에서 종료하도록 선택할 수 있으며 Microsoft Edge는 "페이지 내"가 아닌 탐색이 발생할 때(예: 주소 표시줄, 뒤로 단추 또는 즐겨찾기 링크 사용) Internet Explorer 모드에서 자동으로 종료됩니다.
...
이 정책을 사용하지 않도록 설정하면 사용자는 Internet Explorer 모드에서 구성된 사이트를 다시 로드할 수 없습니다.

이 정책을 사용하도록 설정하면 [InternetExplorerIntegrationTestingAllowed](#internetexplorerintegrationtestingallowed) 정책을 구성한 방법보다 우선하며 해당 정책은 사용하지 않도록 설정됩니다.
...
Internet Explorer 모드에 대한 자세한 내용은 [https://go.microsoft.com/fwlink/?linkid=2094210](https://go.microsoft.com/fwlink/?linkid=2094210)을 참조하세요.
  ...
  - GP 고유 이름: InternetExplorerIntegrationReloadInIEModeAllowed
  - GP 이름: 구성되지 않은 사이트를 Internet Explorer 모드로 다시 로드할 수 있도록 허용
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 관리 템플릿/Microsoft Edge - 기본 설정(사용자 재정의 가능)/
  - GP ADMX 파일 이름: MSEdge.admx
  ...
  - 경로 (필수): SOFTWARE\정책\Microsoft\Edge
  - 경로(권장): SOFTWARE\정책\Microsoft\Edge\권장
  - 값 이름: InternetExplorerIntegrationReloadInIEModeAllowed
  - 값 형식: REG_DWORD
...
```
  ...
  #### <a name="configure-how-frequently-the-enterprise-mode-site-list-is-refreshed"></a>엔터프라이즈 모드 사이트 목록을 새로 고치는 빈도 구성
  ...
  이 설정을 사용하면 엔터프라이즈 모드 사이트 목록에 대한 사용자 지정 새로 고침 간격을 지정할 수 있습니다. 새로 고침 간격은 분 단위로 지정됩니다. 최소 새로 고침 간격은 30분입니다.

이 설정은 [InternetExplorerIntegrationSiteList](#internetexplorerintegrationsitelist) 또는 [InternetExplorerIntegrationCloudSiteList](#internetexplorerintegrationcloudsitelist) 설정이 구성된 경우에만 적용됩니다.

이 정책을 구성하는 경우 Microsoft Edge는 지정된 새로 고침 간격을 사용하여 구성된 엔터프라이즈 모드 사이트 목록의 업데이트된 버전을 검색하려고 시도합니다.

이 정책을 사용하지 않도록 설정하거나 구성하지 않으면 Microsoft Edge는 기본 새로 고침 간격(현재 120분)을 사용합니다.
  ...
  - GP 고유 이름: InternetExplorerIntegrationSiteListRefreshInterval
  - GP 이름: 엔터프라이즈 모드 사이트 목록을 새로 고치는 빈도 구성
  - GP 경로 (필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
  ...
  - 경로 (필수): SOFTWARE\정책\Microsoft\Edge
  - 경로(권장): 해당 없음
  - 값 이름: InternetExplorerIntegrationSiteListRefreshInterval
  - 값 형식: REG_DWORD
...
```
  ...
  #### <a name="allow-internet-explorer-mode-testing-obsolete"></a>Internet Explorer 모드 테스트 허용 (사용되지 않음)

  
  >사용되지 않음: 이 정책은 더 이상 사용되지 않으며 Microsoft Edge 94 이상에서는 작동하지 않습니다.
  #### <a name="supported-versions"></a>지원 버전:

  - Windows 및 macOS(86부터 94까지)

  #### <a name="description"></a>설명

  이 정책은 향상된 기능으로 대체되었으므로 더 이상 사용되지 않습니다. 버전 94 이후의 Microsoft Edge에서는 작동하지 않습니다. 사용자가 Internet Explorer 모드로 응용 프로그램을 열 수 있도록 허용하려면 [InternetExplorerIntegrationReloadInIEModeAllowed](#internetexplorerintegrationreloadiniemodeallowed) 정책을 대신 사용합니다. 또는 사용자는 여전히 --ie-mode-test 플래그를 사용할 수 있습니다.
...
해당 정책을 사용하지 않도록 설정하거나 구성하지 않으면 사용자는 “추가 도구” 메뉴 아래의 'Internet Explorer 모드로 열기' 및 'Edge 모드 에서 열기' 옵션을 볼 수 없습니다. 그러나 사용자는 --ie-모드-테스트 플래그를 사용하여 이러한 옵션을 구성할 수 있습니다.
  ...
  - GP 이름: Internet Explorer 모드 테스트 허용(사용되지 않음)
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
...
```
  ...
  #### <a name="configure-the-pixel-adjustment-between-windowopen-heights-sourced-from-ie-mode-pages-vs-edge-mode-pages"></a>IE 모드 페이지와 Edge 모드 페이지에서 가져온 Window.open 높이 간의 픽셀 조정 구성
  ...
  이 설정을 사용하면 Internet Explorer 모드 사이트에서 창 열기를 통해 생성된 팝업 창의 높이를 사용자 지정하여 조정을 할 수 있습니다.

해당 정책을 구성하면 Microsoft Edge에서 높이에 조정값을 픽셀 단위로 추가합니다. 정확한 차이는 IE와 Edge의 UI 구성에 따라 다르지만 일반적으로 차이값은 5입니다.

이 정책을 사용하지 않도록 설정하거나 구성하지 않으면 Microsoft Edge 창 높이 계산에서 IE 모드 창 열기를 Edge 모드 창 열기와 동일하게 처리합니다.
  ...
  - GP 고유 이름: InternetExplorerIntegrationWindowOpenHeightAdjustment
  - GP 이름: IE 모드 페이지에서 원본되는 창 열기 높이와 Edge 모드 페이지 간의 픽셀 조정 구성
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
  ...
  - 경로 (필수): SOFTWARE\정책\Microsoft\Edge
  - 경로(권장): 해당 없음
  - 값 이름: InternetExplorerIntegrationWindowOpenHeightAdjustment
  - 값 형식: REG_DWORD
...
```
  ...
  #### <a name="configure-the-pixel-adjustment-between-windowopen-widths-sourced-from-ie-mode-pages-vs-edge-mode-pages"></a>IE 모드 페이지와 Edge 모드 페이지에서 가져온 Window.open 너비 간의 픽셀 조정 구성
  ...
  이 설정을 사용하면 Internet Explorer 모드 사이트에서 창 열기를 통해 생성된 팝업 창의 너비에 대해 사용자 지정 조정을 지정할 수 있습니다.

해당 정책을 구성하면 Microsoft Edge에서 너비 조정값을 픽셀 단위로 추가합니다. 정확한 차이는 IE와 Edge의 UI 구성에 따라 다르지만 일반적으로 차이값은 4입니다.

이 정책을 사용하지 않도록 설정하거나 구성하지 않으면 Microsoft Edge 창 너비 계산에서 IE 모드 창 열기를 Edge 모드 창 열기와 동일하게 처리합니다.
  ...
  - GP 고유 이름: InternetExplorerIntegrationWindowOpenWidthAdjustment
  - GP 이름: IE 모드 페이지와 Edge 모드 페이지에서 가져온 창 열기 너비 간 픽셀 조정 구성
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
  ...
  - 경로 (필수): SOFTWARE\정책\Microsoft\Edge
  - 경로(권장): 해당 없음
  - 값 이름: InternetExplorerIntegrationWindowOpenWidthAdjustment
  - 값 형식: REG_DWORD
...
```
  ...
  #### <a name="allow-save-page-as-in-internet-explorer-mode"></a>Internet Explorer 모드로 페이지 저장 허용
  ...
  이 정책은 Internet Explorer 모드에서 ‘다른 이름으로 페이지 저장’ 기능을 사용하도록 설정합니다.
사용자는 이 옵션을 사용하여 현재 페이지를 브라우저에 저장할 수 있습니다. 사용자가 저장된 페이지를 다시 열면 기본 브라우저에 로드됩니다.

이 정책을 사용하도록 설정하면 “다른 이름으로 페이지 저장” 옵션을 “추가 도구”에서 클릭할 수 있습니다.

이 정책을 사용하지 않도록 설정하거나 구성하지 않으면 “추가 도구”에서 “다른 이름으로 페이지 저장” 옵션을 선택할 수 없습니다.

참고: “Ctrl+S” 바로 가기가 작동하려면 Internet Explorer 정책 ‘Internet Explorer 모드에서 확장된 핫 키 사용’을 사용하도록 설정해야 합니다.
...
Internet Explorer 모드에 대한 자세한 내용은 [https://go.microsoft.com/fwlink/?linkid=2094210](https://go.microsoft.com/fwlink/?linkid=2094210)을 참조하세요.
  ...
  - GP 고유 이름: InternetExplorerModeEnableSavePageAs
  - GP 이름: Internet Explorer 모드로 페이지 저장 허용
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
  ...
  - 경로 (필수): SOFTWARE\정책\Microsoft\Edge
  - 경로 (권장): 해당 없음
  - 값 이름: InternetExplorerModeEnableSavePageAs
  - 값 형식: REG_DWORD
...
```
  ...
  #### <a name="show-the-reload-in-internet-explorer-mode-button-in-the-toolbar"></a>도구 모음에서 Internet Explorer 모드로 다시 로드 버튼 표시
  ...
  도구 모음에서 Internet Explorer 모드로 다시 로드 버튼을 표시하려면 이 정책을 설정합니다. 사용자는 edge://settings/appearance를 통해 도구 모음의 버튼을 숨길 수 있습니다.  버튼은 [InternetExplorerIntegrationReloadInIEModeAllowed](#internetexplorerintegrationreloadiniemodeallowed) 정책을 사용하도록 설정되어 있거나 사용자가 "Internet Explorer 모드에서 사이트를 다시 로드하도록 허용"을 사용하도록 선택한 경우에만 도구 모음에 표시됩니다.

이 정책을 사용하도록 설정하면 인터넷 모드에서 다시 로드 버튼이 도구 모음에 고정됩니다.

이 정책을 사용하지 않도록 설정하거나 구성하지 않으면 Internet Explorer 모드로 다시 로드 버튼이 기본적으로 도구 모음에 표시되지 않습니다. 사용자는 edge://settings/appearance에서 Internet Explorer 모드 표시 버튼을 전환할 수 있습니다.
  ...
  - GP 이름: 도구 모음에서 Internet Explorer 모드로 다시 로드 버튼 표시
  - GP 경로 (필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 관리 템플릿/Microsoft Edge - 기본 설정(사용자 재정의 가능)/
  - GP ADMX 파일 이름: MSEdge.admx
...
```
  ...
  격리된 프로세스에서 실행할 원본을 지정합니다.

기본적으로 Microsoft Edge 각 사이트의 페이지를 자체 프로세스로 격리합니다. 이 정책은 사이트 대신 원본에 따라 보다 세부적인 격리를 사용하도록 설정합니다. 예를 들어 https://subdomain.contoso.com/을(를) 지정하면 https://subdomain.contoso.com/의 페이지가 https://contoso.com/ 사이트 내의 다른 원본 페이지와 다른 프로세스에서 격리됩니다.

이 정책을 사용하도록 설정하면 쉼표로 구분된 목록의 명명된 각 원본이 자체 프로세스에서 실행됩니다.

이 정책을 사용하지 않도록 설정하거나 구성하지 않으면 페이지가 사이트별로 격리됩니다.
...
```
  ...
  #### <a name="enable-windows-to-search-local-microsoft-edge-browsing-data"></a>Windows가 로컬 Microsoft Edge 탐색 데이터를 검색하도록 설정
  ...
  Windows에서 사용자 장치에 로컬로 저장된 Microsoft Edge 검색 데이터를 인덱싱할 수 있으며 사용자가 Windows의 작업 표시줄에 있는 검색창과 같은 Windows 기능에서 이전에 저장된 검색 데이터를 직접 찾아서 실행할 수 있습니다.

이 정책을 사용하도록 설정하거나 구성하지 않으면 Microsoft Edge 로컬 검색 데이터를 Windows 인덱서에 게시합니다.

이 정책을 사용하지 않도록 설정하면 Microsoft Edge Windows 인덱서에 데이터를 공유하지 않습니다.

이 정책을 사용하지 않도록 설정하면 Microsoft Edge 장치에서 Windows와 공유되는 데이터를 제거하고 새 검색 데이터 공유를 중지합니다.
  ...
  - GP 고유 이름: LocalBrowserDataShareEnabled
  - GP 이름: Windows가 로컬 Microsoft Edge 탐색 데이터를 검색하도록 설정
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 관리 템플릿/Microsoft Edge - 기본 설정(사용자 재정의 가능)/
  - GP ADMX 파일 이름: MSEdge.admx
  ...
  - 경로 (필수): SOFTWARE\정책\Microsoft\Edge
  - 경로(권장): SOFTWARE\정책\Microsoft\Edge\권장
  - 값 이름: LocalBrowserDataShareEnabled
  - 값 형식: REG_DWORD
...
```
  ...
  #### <a name="always-use-microsoft-autoupdate-as-the-updater-for-microsoft-edge"></a>항상 Microsoft 자동 업데이트를 Microsoft Edge에 대한 업데이트 프로그램으로 사용합니다.
  ...
  이 정책을 사용하면 Microsoft Edge가 사용하는 업데이트 관리자를 구성할 수 있습니다.

이 정책을 사용하도록 설정하면 microsoft 자동 업데이트에서만 Microsoft Edge가 업데이트됩니다.

이 정책을 사용하지 않도록 설정하거나 구성하지 않으면 Microsoft Edge가 Microsoft Edge 업데이트에 의해 업데이트됩니다.
...
이 정책을 사용하도록 설정하거나 구성하지 않으면 사용자는 설정 옵션을 사용하여 컴퓨터에 단일 MSA 계정만 있는 경우 비 MSA 프로필은 시스템에 있는 MSA 자격 증명을 사용하여 Microsoft 사이트에 대한 Single Sign-On을 사용할 수 있도록 합니다.
  ...
  - 경로 (필수): SOFTWARE\정책\Microsoft\Edge
  - 경로(권장): SOFTWARE\정책\Microsoft\Edge\권장
  - 값 이름: MSAWebSiteSSOUsingThisProfileAllowed
  - 값 형식: REG_DWORD
...
```
  ...
  - 기본 설정 키 이름: MSAWebSiteSSOUsingThisProfileAllowed
  - 예제 값:
``` xml
<false/>
```
...
```
  ...
  최대 10 개의 검색 엔진 목록을 구성할 수 있습니다. 이 중 하나는 기본 검색 엔진으로 표시되어야 합니다. Microsoft Edge 버전 100부터 최대 100개의 엔진을 구성할 수 있습니다.
...
Microsoft Edge 83에서 시작하는 선택적 allow_search_engine_discovery 매개 변수를 사용하여 검색 엔진을 검색할 수 있습니다. 해당 매개 변수는 목록의 첫 번째 항목이어야 합니다. allow_search_engine_discovery를 지정하지 않은 경우 기본적으로 검색 엔진을 검색할 수 없습니다. Microsoft Edge 84부터 이 정책을 권장 정책으로 설정하여 검색 공급자 검색을 허용할 수 있습니다. 선택적 Allow_search_engine_discovery 매개 변수는 추가할 필요가 없습니다. Microsoft Edge 100부터 이 정책을 권장 정책으로 설정하면 사용자가 Microsoft Edge 설정에서 새 검색 엔진을 수동으로 추가할 수도 있습니다.
...
```
  ...
  #### <a name="let-users-snip-a-math-problem-and-get-the-solution-with-a-step-by-step-explanation-in-microsoft-edge"></a>사용자가 수학 문제를 캡쳐하고 Microsoft Edge에서 단계별 설명을 통해 해결책을 얻을 수 있도록 함
  ...
  이 정책을 사용하면 사용자가 Microsoft Edge에서 Math Solver 도구를 사용할 수 있는지 여부를 관리할 수 있습니다.

정책을 사용하도록 설정하거나 구성하지 않은 경우 사용자는 수학 문제를 간단히 해결하고 Microsoft Edge 측면 창에서 솔루션에 대한 단계별 설명을 포함하여 솔루션을 얻을 수 있습니다.

정책을 사용하지 않도록 설정하면 Math Solver 도구가 사용하지 않도록 설정되고 사용자가 이를 사용할 수 없습니다.

참고: [ComponentUpdatesEnabled](#componentupdatesenabled) 정책을 사용하지 않도록 설정하면 Math Solver 구성 요소도 사용하지 않도록 설정됩니다.
  ...
  - GP 고유 이름: MathSolverEnabled
  - GP 이름: 사용자가 수학 문제를 캡쳐하고 Microsoft Edge에서 단계별 설명을 통해 해결책을 얻을 수 있도록 함
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
...
```
  ...
  #### <a name="allow-users-to-access-the-microsoft-office-menu"></a>사용자가 Microsoft Office 메뉴에 액세스할 수 있도록 허용
  ...
  Microsoft Office 메뉴에 액세스할 수 있는 경우 Microsoft Word 및 Microsoft Excel과 같은 Office 응용 프로그램에 액세스할 수 있습니다.

이 정책을 사용하도록 설정하거나 구성하지 않으면 Microsoft Office 메뉴를 열 수 있습니다.

이 정책을 사용하지 않도록 설정하면 Microsoft Office 메뉴에 액세스할 수 없습니다.
  ...
  - GP 고유 이름: MicrosoftOfficeMenuEnabled
  - GP 이름: 사용자가 Microsoft Office 메뉴에 액세스할 수 있도록 허용
  - GP 경로 (필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
  ...
  - 경로 (필수): SOFTWARE\정책\Microsoft\Edge
  - 경로 (권장): 해당 없음
  - 값 이름: MicrosoftOfficeMenuEnabled
  - 값 형식: REG_DWORD
...
```
  ...
  - 기본 설정 키 이름: MicrosoftOfficeMenuEnabled
  - 예제 값:
``` xml
<false/>
```
...
```
  ...
  #### <a name="secure-mode-and-certificate-based-digital-signature-validation-in-native-pdf-reader"></a>기본 PDF 판독기에서 보안 모드 및 인증서 기반 디지털 서명 유효성 검사
  ...
  이 정책은 보안 환경에서 PDF 파일에 대한 디지털 서명 유효성 검사를 활성화하여 서명의 올바른 유효성 검사 상태를 표시합니다.

이 정책을 실행하면 인증서 기반 디지털 서명이 있는 PDF 파일이 열리고 보안 수준이 높은 서명의 유효성을 보고 확인할 수 있는 옵션이 제공됩니다.

이 정책을 사용하지 않도록 설정하거나 구성하지 않으면 서명을 보고 확인하는 기능을 사용할 수 없습니다.
  ...
  - GP 고유 이름: PDFSecureMode
  - GP 이름: 기본 PDF 판독기에서 보안 모드 및 인증서 기반 디지털 서명 유효성 검사
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
...
```
  ...
  #### <a name="allow-personalization-of-ads-microsoft-edge-search-news-and-other-microsoft-services-by-sending-browsing-history-favorites-and-collections-usage-and-other-browsing-data-to-microsoft"></a>검색 기록, 즐겨찾기 및 모음, 사용 현황 및 기타 검색 데이터를 Microsoft로 전송하여 광고, Microsoft Edge, 검색, 뉴스 및 기타 Microsoft 서비스의 개인화 허용
  ...
  이 정책은 Microsoft가 광고, 검색, 뉴스, Microsoft Edge 및 기타 Microsoft 서비스를 개인 설정하는 데 사용할 사용자의 Microsoft Edge 검색 기록, 즐겨찾기 및 모음, 사용 현황 및 기타 검색 데이터를 수집하지 못하도록 합니다.

해당 설정은 하위 계정 또는 엔터프라이즈 계정에는 사용할 수 없습니다.
...
해당 정책을 사용하지 않도록 설정하면 사용자는 해당 설정을 변경하거나 재정의할 수 없습니다. 해당 정책을 사용하도록 설정하거나 구성하지 않으면 Microsoft Edge는 사용자의 기본 설정의 기본값이 됩니다.
  ...
  - GP 이름: 검색 기록, 즐겨찾기 및 모음, 사용 현황 및 기타 검색 데이터를 Microsoft로 전송하여 광고, Microsoft Edge, 검색, 뉴스 및 기타 Microsoft 서비스의 개인화 허용
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
...
```
  ...
  #### <a name="enable-proactive-authentication-obsolete"></a>사전 인증 사용(사용되지 않음)

  
  >사용되지 않음: 이 정책은 더 이상 사용되지 않으며 Microsoft Edge 90 이후에는 작동하지 않습니다.
  #### <a name="supported-versions"></a>지원 버전:

  - Windows 및 macOS(77부터 90까지)

  #### <a name="description"></a>설명

  이 정책은 브라우저 로그인과는 독립적으로 작동하지 않기 때문에 더 이상 사용되지 않습니다. 버전 90 이후의 Microsoft Edge에서 작동하지 않습니다. 브라우저 로그인을 구성하려면 [BrowserSignin](#browsersignin) 정책을 사용합니다.
...
```
  ...
  #### <a name="prompt-the-user-to-select-a-certificate-when-multiple-certificates-match"></a>여러 인증서가 일치하는 경우 인증서를 선택하라는 메시지를 표시합니다.
  ...
  이 정책은 둘 이상의 인증서가 [AutoSelectCertificateForUrls](#autoselectcertificateforurls)와 일치하는 경우 사용자에게 클라이언트 인증서를 선택하라는 메시지를 표시할지 여부를 제어합니다.
이 정책을 True로 설정하면 자동 선택 정책이 여러 인증서와 일치할 때마다 클라이언트 인증서를 선택하라는 메시지가 표시됩니다.
이 정책이 False로 설정되거나 설정되지 않은 경우 자동 선택과 일치하는 인증서가 없는 경우에만 사용자에게 메시지가 표시될 수 있습니다.
  ...
  - GP 고유 이름: PromptOnMultipleMatchingCertificates
  - GP 이름: 여러 인증서가 일치하는 경우 인증서를 선택하라는 메시지를 표시합니다.
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
  ...
  - 경로 (필수): SOFTWARE\정책\Microsoft\Edge
  - 경로 (권장): 해당 없음
  - 값 이름: PromptOnMultipleMatchingCertificates
  - 값 형식: REG_DWORD
...
```
  ...
  - 기본 설정 키 이름: PromptOnMultipleMatchingCertificates
  - 예를 들어 값:
``` xml
<true/>
```
...
```
  ...
  사용자가 OneDrive 또는 SharePoint에 없는 웹에서 공개적으로 액세스할 수 있는 Office 파일을 볼 수 있는지 여부를 설정할 수 있습니다. (예: Word 문서, PowerPoint 프레젠테이션 및 Excel 스프레드시트)
...
Microsoft Edge 주 릴리스 87부터 이 정책을 'Sitelist'로 설정하면 Internet Explorer(IE)는 최신 브라우저가 필요한 사이트를 Microsoft Edge로 리디렉션합니다. (참고: Sitelist 설정은 '호환되지 않는 사이트 목록을 기반으로 사이트 리디렉션', 값 1)입니다.

Internet Explorer에서 Microsoft Edge로 사이트를 리디렉션할 때 이전 콘텐츠가 없으면 사이트 로드를 시작한 Internet Explorer 탭이 닫힙니다. 그렇지 않으면 사이트가 Microsoft Edge로 리디렉션된 이유를 설명하는 Microsoft 도움말 페이지로 이동합니다. IE 사이트를 로드하기 위해 Microsoft Edge를 시작하면 최신 브라우저에서 사이트가 가장 잘 작동한다는 알림 표시줄이 표시됩니다.

모든 탐색을 리디렉션하려면 Internet Explorer 11 실행 중지 정책을 구성하여 모든 탐색을 IE11에서 Microsoft Edge로 리디렉션할 수 있습니다. 또한 첫 번째 실행 후 IE11 앱 아이콘을 사용자에게 숨깁니다.

이 정책을 구성하지 않은 경우:
  - Microsoft Edge 주 릴리스 87부터는 정책을 'Sitelist'로 설정하는 것과 동일한 경험을 하게 됩니다. Internet Explorer는 최신 브라우저가 필요한 사이트를 Microsoft Edge로 리디렉션합니다.
  - 나중에 조직의 기본값이 모든 탐색을 자동으로 리디렉션하도록 변경될 수 있습니다. 자동 리디렉션하지 않으려면 이 정책을 '사용 안 함' 또는 'Sitelist'로 설정합니다.
...
```
  ...
  #### <a name="configure-related-matches-in-find-on-page"></a>페이지에서 찾기에서 관련 일치 구성
  ...
  사용자가 Microsoft Edge에서 맞춤법 검사, 동의어 및 Q&A 결과를 제공하는 페이지에서 관련 일치를 수신하는 방법을 지정합니다.

이 정책을 사용하거나 구성하지 않으면 모든 사이트의 페이지에서 찾기에서 관련 일치 항목을 수신할 수 있습니다. 결과는 클라우드 서비스에서 처리됩니다.

이 정책을 사용하지 않도록 설정하면 사용자가 제한된 사이트의 페이지에서 찾기에서 관련 일치 항목을 수신할 수 있습니다. 결과는 사용자의 디바이스에서 처리됩니다.
  ...
  - GP 고유 이름: RelatedMatchesCloudServiceEnabled
  - GP 이름: 페이지에서 찾기에서 관련 일치 구성
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
  ...
  - 경로 (필수): SOFTWARE\정책\Microsoft\Edge
  - 경로 (권장): 해당 없음
  - 값 이름: RelatedMatchesCloudServiceEnabled
  - 값 형식: REG_DWORD
...
```
  ...
  - 기본 설정 키 이름: RelatedMatchesCloudServiceEnabled
  - 예를 들어 값:
``` xml
<true/>
```
...
```
  ...
  #### <a name="set-the-time-interval-for-relaunch"></a>다시 시작 시간 간격 설정
  ...
  다시 시작 알림 기간 종료에 대한 대상 시간 창을 지정합니다.
...
경고: 이 정책을 설정하면 소프트웨어 업데이트 적용이 지연될 수 있습니다.
  ...
  - GP 고유 이름: RelaunchWindow
  - GP 이름: 다시 시작 시간 간격 설정
  - GP 경로 (필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
...
```
  ...
  #### <a name="allow-remote-debugging"></a>원격 디버깅 허용
...
이 정책을 사용하거나 구성하지 않으면 사용자가 --remote-debug-port 및 --remote-debugging-pipe 명령줄 스위치를 지정하여 원격 디버깅을 사용할 수 있습니다.

이 정책을 사용하지 않도록 설정하면 사용자가 원격 디버깅을 사용할 수 없습니다.
  ...
  - GP 고유 이름: RemoteDebuggingAllowed
  - GP 이름: 원격 디버깅 허용
  - GP 경로 (필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
...
```
  ...
  - 기본 설정 키 이름: RemoteDebuggingAllowed
  - 예제 값:
``` xml
<true/>
```
...
이 정책은 Windows 10 디바이스에서 지원되지 않습니다.
...
```
  ...
  #### <a name="extend-adobe-flash-content-setting-to-all-content-obsolete"></a>Adobe Flash 콘텐츠 설정을 모든 콘텐츠로 확장(사용되지 않음)
...
이 새 제한의 영향을 받는 내부 웹 사이트를 업데이트하는 데 더 많은 시간이 필요한 관리자가 이 기능을 사용할 수 있습니다. 이 엔터프라이즈 정책은 일시적입니다. Microsoft Edge 버전 117 이후에 제거됩니다.
...
```
  ...
  Microsoft Edge에는 하드웨어 적용 스택 보호 보안 기능이 포함되어 있습니다. 이 기능을 사용하면 브라우저의 보안을 손상시키려는 시도가 아닌 경우 예기치 않게 브라우저가 충돌할 수 있습니다.

이 정책을 사용하면 이 기능에 의해 트리거된 충돌이 발생한 후 하드웨어 적용 스택 보호 기능의 동작을 제어할 수 있습니다.

이 기능을 사용하지 않으려면 이 정책을 'Disable'로 설정합니다.

다음에 Microsoft Edge가 업데이트될 때까지 기능을 사용하지 않으려면 이 정책을 'DisableUntilUpdate'로 설정합니다.

이 기능을 계속 사용하려면 이 정책을 'Enable'로 설정합니다.

정책 옵션 매핑:

* 사용 안 하게 (0) = 하드웨어 적용 스택 보호 사용 안함

* DisableUntilUpdate (1) = 다음 Microsoft Edge 업데이트가 있을 때까지 하드웨어 적용 스택 보호를 사용하지 않음.

* 사용 (2) = 하드웨어 적용 스택 보호 사용
...
이 정책을 구성할 시 위의 정보를 사용합니다.
  ...
  - GP 고유 이름: ShadowStackCrashRollbackBehavior
  - GP 이름: ShadowStack 크래시 롤백 동작 구성
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
  ...
  - 경로 (필수): SOFTWARE\정책\Microsoft\Edge
  - 경로(권장): 해당 없음
  - 값 이름: ShadowStackCrashRollbackBehavior
  - 값 형식: REG_DWORD
...
```
  ...
  #### <a name="specifies-whether-sharedarraybuffers-can-be-used-in-a-non-cross-origin-isolated-context"></a>cross-origin-isolated가 아닌 컨텍스트에서 SharedArrayBuffers를 사용할 수 있는지 여부 지정
  ...
  cross-origin-isolated가 아닌 컨텍스트에서 SharedArrayBuffers를 사용할 수 있는지 여부 지정  SharedArrayBuffer는 공유 메모리에서 보기를 만드는 데 사용할 수 있는 이진 데이터 버퍼입니다.  SharedArrayBuffer에는 여러 인기 CPU에서 메모리 액세스 취약성이 있습니다.

이 정책을 사용하도록 설정하면 사이트에서 제한 없이 SharedArrayBuffers를 사용할 수 있습니다.

이 정책을 사용하지 않도록 설정하거나 구성하지 않으면 사이트에서 원본 간 격리된 경우에만 SharedArrayBuffers를 사용할 수 있습니다.

웹 호환성을 위해 Microsoft Edge 91 이상에서 SharedArrayBuffers를 사용할 때 Microsoft Edge는 교차 원본을 격리해야 합니다.
  ...
  - GP 고유 이름: SharedArrayBufferUnrestrictedAccessAllowed
  - GP 이름: cross-origin-isolated가 아닌 컨텍스트에서 SharedArrayBuffers를 사용할 수 있는지 여부 지정
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
  ...
  - 경로 (필수): SOFTWARE\정책\Microsoft\Edge
  - 경로(권장): 해당 없음
  - 값 이름: SharedArrayBufferUnrestrictedAccessAllowed
  - 값 형식: REG_DWORD
...
```
  ...
  - 기본 설정 키 이름: SharedArrayBufferUnrestrictedAccessAllowed
  - 예제 값:
``` xml
<true/>
```
...
```
  ...
  #### <a name="allow-users-to-configure-site-safety-services"></a>사용자가 Family Safety를 구성할 수 있도록 허용
  ...
  이 정책은 사이트 안전 서비스가 페이지 정보 대화 상자에 상위 사이트 정보를 표시하지 않도록 설정합니다.

이 정책을 사용하도록 설정하거나 구성하지 않으면 상위 사이트 정보가 표시됩니다.

이 정책을 사용하지 않도록 설정하면 상위 사이트 정보가 표시되지 않습니다.
  ...
  - GP 고유 이름: SiteSafetyServicesEnabled
  ...
  - 경로 (필수): SOFTWARE\정책\Microsoft\Edge
  - 경로(권장): SOFTWARE\Policies\Microsoft\Edge\Recommended
  - 값 이름: SiteSafetyServicesEnabled
  - 값 형식: REG_DWORD
...
```
  ...
  - 기본 설정 키 이름: SiteSafetyServicesEnabled
  - 예를 들어 값:
``` xml
<true/>
```
...
```
  ...
  #### <a name="allow-freezing-of-background-tabs-obsolete"></a>배경 탭을 동결할 수 있도록 허용(사용되지 않음)

  
  >OBSOLETE: 이 정책은 더 이상 사용되지 않으며 Microsoft Edge 86 이후에는 작동하지 않습니다.
  #### <a name="supported-versions"></a>지원 버전:

  - Windows 및 macOS(79부터 86까지)

  #### <a name="description"></a>설명

  이 정책은 작동하지 않습니다. [SleepingTabsEnabled](#sleepingtabsenabled)를 대신 사용하세요.
...
해당 정책을 사용하지 않으면 탭이 고정되지 않습니다.
  ...
  - GP 이름: 배경 탭을 동결할 수 있도록 허용(사용되지 않음)
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
...
```
  ...
  #### <a name="enable-travel-assistance"></a>여행 지원 사용
  ...
  여행 지원을 허용/허용하지 않도록 이 정책을 구성합니다.

여행 지원 기능은 브라우저 내에서 여행 관련 작업을 수행하는 사용자에게 유용한 관련 정보를 제공합니다. 이 기능은 Microsoft에서 수집한 여러 원본에서 사용자에게 신뢰할 수 있고 검증된 제안/정보를 제공합니다.

이 설정을 사용하도록 설정하거나 구성하지 않으면 사용자가 여행 관련 작업을 수행할 때 여행 지원이 사용하도록 설정됩니다.

이 설정을 사용하지 않도록 설정하면 여행 지원이 사용하지 않도록 설정되고 사용자는 여행 관련 권장 사항을 볼 수 없습니다.
  ...
  - GP 고유 이름: TravelAssistanceEnabled
  - GP 이름: 여행 지원 사용
  - GP 경로 (필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 관리 템플릿/Microsoft Edge - 기본 설정(사용자 재정의 가능)/
  - GP ADMX 파일 이름: MSEdge.admx
...
```
  ...
  - 기본 설정 키 이름: TravelAssistanceEnabled
  - 예제 값:
``` xml
<true/>
```
  ...
  #### <a name="enable-3des-cipher-suites-in-tls"></a>TLS에서 3DES 암호화 제품군을 사용하도록 설정
  ...
  '경고: 3DES는 버전 95(2021년 10월경)에서 Microsoft Edge로부터 완전히 제거되고 해당 정책은 작동을 중지합니다.

정책이 true로 설정된 경우 TLS의 3DES 암호화 제품군이 사용하도록 설정됩니다. false로 설정하면 사용하지 않도록 설정됩니다. 정책이 설정되지 않은 경우 3DES 암호화 제품군은 기본적으로 사용하지 않도록 설정됩니다. 이 정책은 일시적으로 기한이 지난 서버와의 호환성을 유지하는 데 사용될 수 있습니다. 이는 stopgap 측정값으로, 서버를 다시 구성해야 합니다.
  ...
  - GP 고유 이름: TripleDESEnabled
  - GP 이름: TLS에서 3DES 암호화 제품군을 사용하도록 설정
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
...
```
  ...
  #### <a name="enable-the-user-agent-client-hints-feature-obsolete"></a>사용자 에이전트 클라이언트 힌트 기능 사용(사용되지 않음)

  
  >사용되지 않음: 이 정책은 더 이상 사용되지 않으며 Microsoft Edge 93 이상에서는 작동하지 않습니다.
  #### <a name="supported-versions"></a>지원 버전:

  - Windows 및 macOS(86부터 93까지)

  #### <a name="description"></a>설명

  이 정책은 단기 적응 목적으로만 사용되었으므로 더 이상 사용되지 않습니다. 버전 93 이후의 Microsoft Edge에서는 작동하지 않습니다.
...
```
  ...
  #### <a name="enable-or-disable-the-user-agent-reduction"></a>사용자 에이전트 감소 사용 또는 사용 안 함
  ...
  사용자-에이전트 HTTP 요청 헤더가 축소되도록 예약되어 있습니다. 테스트 및 호환성을 높이기 위해 이 정책은 모든 웹 사이트에 대해 축소 기능을 사용하거나 원본 평가판 또는 현장 평가판 기능을 사용하지 않도록 설정할 수 있습니다.

이 정책을 구성하거나 기본값으로 설정하지 않으면 사용자-에이전트가 실험에 의해 제어됩니다.

사용자-에이전트 요청 헤더의 축소 버전을 강제로 적용하려면 이 정책을 'ForceEnabled'로 설정합니다.

사용자-에이전트 요청 헤더의 전체 버전을 강제로 적용하려면 이 정책을 'ForceDisabled'로 설정합니다.

사용자-에이전트 문자열에 대한 자세한 내용은 다음을 참조하세요.
...
* 기본값(0) = User-Agent 감소는 Experimentation을 통해 제어할 수 있음

* ForceDisabled (1) = 사용자-에이전트 감소가 사용 불가능하며 Experimentation에서 사용하도록 설정하지 않음

* ForceEnabled (2) = 모든 원본에 대해 사용자-에이전트 감소가 사용하도록 설정됨
...
이 정책을 구성할 시 위의 정보를 사용합니다.
  ...
  - GP 고유 이름: UserAgentReduction
  - GP 이름: 사용자-에이전트 감소 사용 또는 사용 안 함
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
...
```
  ...
  #### <a name="visual-search-enabled"></a>비주얼 검색 사용
  ...
  시각적 검색을 사용하면 이미지의 엔터티에 대한 보다 관련성 있는 콘텐츠를 빠르게 탐색할 수 있습니다.

해당 정책을 사용하도록 설정하거나 구성하지 않는 경우 이미지에 커서 올리기, 상황에 맞는 메뉴 및 사이드바에서 검색을 통해 시각적 검색을 사용할 수 있습니다.

해당 정책을 사용하지 않도록 설정하면 시각적 검색이 사용하지 않도록 설정되며 이미지에 마우스 올리기, 상황에 맞는 메뉴 및 사이드바에서 검색을 통해 이미지에 대한 자세한 정보를 얻을 수 없습니다.
...
```
  ...
  #### <a name="allow-webdriver-to-override-incompatible-policies-obsolete"></a>WebDriver가 호환되지 않는 정책을 재정의하도록 허용(사용되지 않음)
...
```
  ...
  #### <a name="enable-support-for-windows-os-routing-table-rules-when-making-peer-to-peer-connections-via-webrtc"></a>WebRTC를 통해 피어 투 피어 연결을 만들 때 Windows OS 라우팅 테이블 규칙 지원 사용
...
이 정책을 사용하도록 설정하면 WebRTC는 라우팅 테이블에 표시된 대로 원격 주소에 대해 표시된 네트워크 인터페이스를 사용하여 피어 투 피어 연결을 만드는 것을 선호합니다.

이 정책은 Windows에서만 사용할 수 있습니다.
  ...
  - GP 고유 이름: WebRtcRespectOsRoutingTableEnabled
  - GP 이름: WebRTC를 통해 피어 투 피어 연결을 만들 때 Windows OS 라우팅 테이블 규칙 지원 사용
  - GP 경로 (필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
  ...
  - 경로 (필수): SOFTWARE\정책\Microsoft\Edge
  - 경로(권장): 해당 없음
  - 값 이름: WebRtcRespectOsRoutingTableEnabled
  - 값 형식: REG_DWORD
...
```
  ...
  #### <a name="force-websql-in-third-party-contexts-to-be-re-enabled-obsolete"></a>타사 컨텍스트에서 WebSQL을 다시 사용하도록 설정(사용되지 않음)

  
  >사용되지 않음: 이 정책은 더 이상 사용되지 않으며 Microsoft Edge 100 이상에서는 작동하지 않습니다.
  #### <a name="supported-versions"></a>지원 버전:

  - Windows 및 MacOS 97~100

  #### <a name="description"></a>설명

  이 정책은 타사 컨텍스트에서 WebSQL을 사용하지 않도록 설정하기 위한 변경 내용과 호환되지 않는 것으로 확인되면 기업에서 웹 콘텐츠를 업데이트할 시간을 더 주기 위한 단기 메커니즘이므로 더 이상 사용되지 않습니다. 버전 100 이후의 Microsoft Edge에서는 작동하지 않습니다.
...
이 정책을 사용하지 않도록 설정하거나 구성하지 않으면 타사 컨텍스트의 WebSQL이 꺼집니다.
  ...
  - GP 이름: 타사 컨텍스트에서 WebSQL을 다시 사용하도록 설정(사용되지 않음)
  - GP 경로(필수): 관리 템플릿/Microsoft Edge/
  - GP 경로 (권장): 해당 없음
  - GP ADMX 파일 이름: MSEdge.admx
...
```
  ...
  #### <a name="enable-the-edge-bar"></a>Edge 막대 사용
  ...
  Edge 막대를 사용하도록 설정합니다. 해당 설정을 사용하면 사용자는 Edge 막대를 사용하여 바탕 화면 또는 응용 프로그램에서 웹을 검색할 수 있습니다. Edge 막대는 웹 제안을 표시하고 Microsoft Edge에서 모든 웹 검색을 여는 검색창을 제공합니다. 검색 상자는 검색(Bing에서 제공) 및 URL 제안을 제공합니다. Edge 막대에는 사용자가 새 Microsoft Edge 브라우저 탭 또는 창에서 msn.com에 대한 자세한 정보를 보기 위해 클릭할 수 있는 피드 타일도 포함되어 있습니다. 피드 타일에 광고가 포함될 수 있습니다. Edge 막대는 Microsoft Edge 설정 또는 Microsoft Edge의 “기타 도구” 메뉴에서 실행할 수 있습니다.

해당 정책을 사용하도록 설정하거나 구성하지 않으면 Edge 막대가 모든 프로필에서 자동으로 사용하도록 설정됩니다.
Microsoft Edge 설정에서 사용자에게 Edge 막대를 실행하는 옵션이 표시됩니다.
Microsoft Edge 설정에서 사용자에게 Windows 시작 시 Edge 막대를 실행(자동 시작)하는 메뉴 항목이 표시됩니다.
[WebWidgetIsEnabledOnStartup](#webwidgetisenabledonstartup) 정책을 사용하는 경우 시작 시 Edge 막대를 사용하는 옵션이 사용하도록 설정됩니다.
[WebWidgetIsEnabledOnStartup](#webwidgetisenabledonstartup) 정책을 사용하지 않도록 설정하거나 구성하지 않으면 시작 시 Edge 막대를 사용하는 옵션이 사용하지 않도록 설정됩니다.
사용자에게 Microsoft Edge "기타 도구" 메뉴에서 Edge 막대를 실행하는 메뉴 항목이 표시됩니다. 사용자는 "기타 도구"에서 Edge 막대를 실행할 수 있습니다.
시스템 트레이의 "종료" 옵션을 사용하거나 작업 표시줄에서 직접 종료하여 Edge 막대를 사용하지 않도록 할 수 있습니다. 자동 시작 옵션을 사용하는 경우 시스템 재부팅 시 Edge 막대가 다시 실행됩니다.

해당 정책을 사용하지 않도록 설정하는 경우 모든 프로필에서 Edge 막대를 사용하지 않습니다.
Microsoft Edge 설정의 Edge 막대를 실행하는 옵션이 사용하지 않도록 설정됩니다.
Windows 시작 시 Edge 막대를 실행하는 옵션(자동 시작)이 사용하지 않도록 설정됩니다.
Microsoft Edge “기타 도구” 메뉴에서 Edge 막대를 실행하는 옵션이 사용하지 않도록 설정됩니다.
...
```
  ...
  #### <a name="allow-the-edge-bar-at-windows-startup"></a>Windows 시작 시 Edge 막대 허용
  ...
  Windows 시작 시 Edge 막대가 실행되도록 허용합니다.

사용하도록 설정하는 경우 Windows 시작 시 Edge 막대가 기본적으로 실행됩니다.
[WebWidgetAllowed](#webwidgetallowed) 정책을 통해 Edge 막대를 사용하지 않도록 설정하는 경우 해당 정책은 Windows 시작 시 Edge 막대를 실행하지 않습니다.

해당 정책을 사용하지 않도록 설정하는 경우 Windows 시작 시 모든 프로필에서 Edge 막대를 실행하지 않습니다.
Windows 시작 시 Edge 막대를 실행하는 옵션이 Microsoft Edge 설정에서 사용하지 않도록 설정되며 해제됩니다.

해당 정책을 구성하지 않으면 Windows 시작 시 모든 프로필에서 Edge 막대를 실행하지 않습니다.
Windows 시작 시 Edge 막대를 실행하는 옵션은 Microsoft Edge 설정에서 해제됩니다.
...
```
  ...
  #### <a name="use-windows-proxy-resolver"></a>Windows 프록시 해결 프로그램 사용
  ...
  이 정책은 이후 릴리스에서 유사한 기능으로 대체됩니다. 자세한 내용은 https://crbug.com/1032820을 참조하세요.
