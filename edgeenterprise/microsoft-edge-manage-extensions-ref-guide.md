...
ms.locfileid: "12297936"
---
# <a name="a-detailed-guide-to-configuring-extensions-using-the-extensionsettings-policy"></a>ExtensionSettings 정책을 사용하여 확장을 구성하기 위한 자세한 가이드
...
ExtensionSettings 정책에서 모든 확장 관리 설정을 설정할지 아니면 다른 정책을 통해 이러한 컨트롤을 설정할지 여부를 결정하십시오.
...
이 정책은 초기 설치를 위해 확장을 다운로드할 업데이트 URL, 차단된 사용 권한 등의 설정을 제어할 수 있습니다. 이 정책을 사용하여 실행이 허용되지 않는 사용 권한을 식별할 수 있습니다. 사용 가능한 정책 필드는 다음 표에 설명되어 있습니다.
...
| **override_update_url**| 93 Microsoft Edge 사용 가능<br>이 필드를 로 설정하면 Microsoft Edge 확장 업데이트에 ExtensionSettings 정책 또는 ExtensionInstallForcelist 정책에 지정된 업데이트 `true` URL을 사용합니다.<br>이 필드가 설정되지 않은 경우 또는 로 Microsoft Edge 확장의 매니페스트에 지정된 `false` URL을 업데이트에 사용합니다.|
| **toolbar_state**| 94 Microsoft Edge 사용 가능<br>이 정책 설정을 사용하면 도구 모음에 설치된 확장을 강제로 표시 할 수 있습니다. 기본 상태는 `default_shown` 모든 확장에 대한 것입니다. 이 설정에 대해 다음과 같은 상태일 수 있습니다.<br>-`force_shown`: 도구 모음에 설치된 확장을 강제로 표시하는 것을 선택할 수 있습니다. 사용자는 도구 모음에서 지정된 확장 아이콘을 숨길 수 없습니다.<br>-`default_hidden`: 이 상태의 경우 설치 시 도구 모음에서 확장이 숨겨집니다. 사용자는 필요한 경우 도구 모음에 표시될 수 있습니다.<br>-`default_shown`: 브라우저에 설치된 모든 확장의 기본 설정입니다.

전역 범위(*)에서 다음 키를 허용합니다.

- blocked_permissions
- installation_mode , 또는 이 범위의 유효한 `"blocked"` `"allowed"` `"removed"` 값입니다.
...
업데이트 URL 범위에서는 다음 키를 사용할 수 있습니다.
...
1. 그룹 정책 편집기를 열고 확장 Microsoft Edge > > 정책 **구성으로 이동합니다.**
2. 정책을 사용하도록 설정하고 텍스트 상자에 줄바락 없이 한 줄로 압축된 JSON(JavaScript Object Notation) 데이터를 입력합니다.
...
이 정책의 두 부분, 즉 기본 범위와 개별 범위를 이해해야 합니다. 기본 범위는 자체 범위가 없는 확장에 대한 catch-all입니다. 개별 범위는 해당 확장에만 적용됩니다.  
...
이전 예제에서 "normal_installed" 대신 "force_installed"를 사용하면 사용자 조작 없이 확장이 자동으로 설치되지만 확장을 사용하지 않도록 설정할 수 있습니다.  
...
다음으로 만들 키는 개별 범위의 확장 ID 또는 기본 범위의 별표(*)입니다. 예를 들어 레지스트리에서 Google Hangouts에 적용되는 설정에 대해 다음 위치를 사용합니다.

`HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Edge\ExtensionSettings\nckgahadagoaajjgafhacjanaoiihapd`

기본 범위(추가)에 적용되는 설정의 경우 레지스트리에서 다음 위치를 사용합니다.
